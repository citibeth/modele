...
!@ver  1.0
     ...
     .                    ,pp2diat_day,pp2diat_day_glob
     .                    ,pp2chlo_day,pp2chlo_day_glob
     .                    ,pp2cyan_day,pp2cyan_day_glob
     .                    ,pp2cocc_day,pp2cocc_day_glob
     ...
     .                 ,ij_DICrhs5,ij_DICrhs6,ij_xchl,ij_sunz,ij_solz
     .                 ,ij_pp1,ij_pp2,ij_pp3,ij_pp4
     .                 ,ijl_avgq,oijl=>oijl_loc
     .                 ,ij_NITrhs1,ij_NITrhs2,ij_NITrhs3,ij_NITrhs4
     .                 ,ij_NITrhs5,ij_NITrhs6
     .                 ,ij_AMMrhs1
     .                 ,ij_chlrhs1,ij_chlrhs2,ij_chlrhs3,ij_chlrhs4
     .                 ,ij_chlrhs5
     .                 ,ij_HERBrhs1,ij_HERBrhs2
     .                 ,ij_NDETrhs1,ij_NDETrhs2,ij_NDETrhs3,ij_NDETrhs4
     .                 ,ij_NDETrhs5
     .                 ,ijl_kpar
     .                 ,ijl_dtemp
!    .                 ,ijl_edz,ijl_esz,ijl_euz
#ifndef TRACERS_GASEXCH_ocean_CO2
#ifdef TRACERS_OceanBiology
     .                 ,ij_flux
#endif
     ...
     .    ,cexp_loc=>cexpij, cexpav_loc
     .    ,pp2tot_day_loc=>pp2tot_day,pp2tot_dayav_loc
     .    ,pp2diat_day_loc=>pp2diat_day,pp2diat_dayav_loc
     .    ,pp2chlo_day_loc=>pp2chlo_day,pp2chlo_dayav_loc
     .    ,pp2cyan_day_loc=>pp2cyan_day,pp2cyan_dayav_loc
     .    ,pp2cocc_day_loc=>pp2cocc_day,pp2cocc_dayav_loc
     .    ,pCO2_loc=>pCO2,pCO2av_loc
#ifdef TRACERS_Alkalinity
     ...
     .     pCO2av_loc,
     .     cexpav,caexpav,cexpij,
     .     cexpav_loc,caexpav_loc,
     .     pp2tot_dayav,pp2tot_dayav_loc,
     .     pp2diat_dayav,pp2diat_dayav_loc,
     .     pp2chlo_dayav,pp2chlo_dayav_loc,
     .     pp2cyan_dayav,pp2cyan_dayav_loc,
     .     pp2cocc_dayav,pp2cocc_dayav_loc
     .    ,pHav,pHav_loc
#endif
...
#else
      if (nstep.eq.1) then
        if (AM_I_ROOT()) print*, 'COLD INITIALIZATION1...'
        trcout=.true.
#endif
...
!Warm initialization
      ...
      if (AM_I_ROOT()) print*,'nstep0,itimei,nstep=',nstep0,itimei,nstep
         call obio_init
...
#ifdef OBIO_TRANSIENT
      write(*,'(a)')'For restart-transient runs.....'
      !!write(*,'(a,2i9,f10.3)')
      write(*,*)
     .            'nstep0,nstep,time=',nstep0,nstep,time
      if (nstep0.eq.0.and.nstep.eq.1) then
         call obio_init
         print*,'WARM INITIALIZATION'
         call obio_trint(0)
      endif
#endif      /* OBIO_TRANSIENT */
...
#ifdef OBIO_ON_GARYocean
       if (i.eq.120.and.j.eq.120) vrbos=.true.   !trop Atl
       if (i.eq.120.and.j.eq.140) vrbos=.true.   !subtrop Atl
       if (i.eq.50.and.j.eq.90) vrbos=.true.     !equator Pac
       if (i.eq.270.and.j.eq.140) vrbos=.true.   !NW Pacific
       if (i.eq.120.and.j.eq.30) vrbos=.true.    !ACC region, off Weddell
#else
       if (i.eq.182.and.j.eq.321) vrbos=.true.   !N Atlantic
       if (i.eq.352.and.j.eq.194) vrbos=.true.   !Antarctic-Weddell Sea
       if (i.eq.170.and.j.eq.152) vrbos=.true.   !NW Pacific
       if (i.eq.239.and.j.eq.266) vrbos=.true.   !Eq Pacific 
       if (i.eq.234.and.j.eq.342) vrbos=.true.   !Eq Atlantic
#endif
...
!    .             ,temp1d(k),saln1d(k),dp1d(k),rho_water
     ...
     .                                *  MO(I,J,k)*DXYPO(J)           ! kg,trac/kg,air=> kg,trac
       ...
       tracer(i,j,1,1) = max(tracer(i,j,1,1),surfN_ij)
#else
       tracer(i,j,1,1) = surfN_ij
#endif
#endif
...
cdiag write(*,'(a,4i5)')'nstep,i,j,kmax= ',nstep,i,j,kmax
      ...
      !for hycom we define it differently, noting that profiles in hycom are shallower
      kzc = 1
      do k=kmax+1,1,-1
...
#ifdef KPAR_2_OCEAN
       do k=1,kdm
!      do ichan = 1,nlt   !sum over wavelengths
!      OIJL(I,J,k,IJL_edz ) = OIJL(I,J,k,IJL_edz ) +  Edz(ichan,k) 
!      OIJL(I,J,k,IJL_esz ) = OIJL(I,J,k,IJL_esz ) +  Esz(ichan,k) 
!      OIJL(I,J,k,IJL_euz ) = OIJL(I,J,k,IJL_euz ) +  Euz(ichan,k) 
!      enddo
!            if (vrbos) write(*,'(a,i9,3i5,e12.4)')'kpar2=',
!    .           nstep,i,j,k,kpar(k)
...
#ifdef OBIO_ON_GARYocean
      OIJ(I,J,IJ_NITrhs1)=OIJ(I,J,IJ_NITrhs1)+rhs_obio(i,j,1,5)
     .     +rhs_obio(i,j,1,6)+rhs_obio(i,j,1,7)+rhs_obio(i,j,1,8)    !nitr uptake
      OIJ(I,J,IJ_NITrhs2)=OIJ(I,J,IJ_NITrhs2)+rhs_obio(i,j,1,11)
      OIJ(I,J,IJ_NITrhs3)=OIJ(I,J,IJ_NITrhs3)+rhs_obio(i,j,1,9)
      OIJ(I,J,IJ_NITrhs4)=OIJ(I,J,IJ_NITrhs4)+rhs_obio(i,j,1,15)
      OIJ(I,J,IJ_NITrhs5)=OIJ(I,J,IJ_NITrhs5)+rhs_obio(i,j,1,14)
      OIJ(I,J,IJ_NITrhs6)=OIJ(I,J,IJ_NITrhs6)+rhs_obio(i,j,1,10)

      OIJ(I,J,IJ_AMMrhs1)=OIJ(I,J,IJ_AMMrhs1)+rhs_obio(i,j,2,5)
     .     +rhs_obio(i,j,2,6)+rhs_obio(i,j,2,7)+rhs_obio(i,j,2,8)    !ammo uptake

      OIJ(I,J,IJ_chlrhs1)=OIJ(I,J,IJ_chlrhs1)+rhs_obio(i,j,5,13)
     .     +rhs_obio(i,j,6,13)+rhs_obio(i,j,7,13)+rhs_obio(i,j,8,13)
      OIJ(I,J,IJ_chlrhs2)=OIJ(I,J,IJ_chlrhs2)+rhs_obio(i,j,7,12)
      OIJ(I,J,IJ_chlrhs3)=OIJ(I,J,IJ_chlrhs3)+rhs_obio(i,j,5,14)
     .     +rhs_obio(i,j,6,14)+rhs_obio(i,j,7,14)+rhs_obio(i,j,8,14)
      OIJ(I,J,IJ_chlrhs4)=OIJ(I,J,IJ_chlrhs4)+rhs_obio(i,j,5,15)
     .     +rhs_obio(i,j,6,15)+rhs_obio(i,j,7,15)+rhs_obio(i,j,8,15)
      OIJ(I,J,IJ_chlrhs5)=OIJ(I,J,IJ_chlrhs5)+rhs_obio(i,j,5,5)
     .     +rhs_obio(i,j,6,6)+rhs_obio(i,j,7,7)+rhs_obio(i,j,8,8)

      OIJ(I,J,IJ_HERBrhs1)=OIJ(I,J,IJ_HERBrhs1)+rhs_obio(i,j,9,11)
      OIJ(I,J,IJ_HERBrhs2)=OIJ(I,J,IJ_HERBrhs2)+rhs_obio(i,j,9,14)

      OIJ(I,J,IJ_NDETrhs1)=OIJ(I,J,IJ_NDETrhs1)+rhs_obio(i,j,10,5)
      OIJ(I,J,IJ_NDETrhs2)=OIJ(I,J,IJ_NDETrhs2)+rhs_obio(i,j,10,7)
      OIJ(I,J,IJ_NDETrhs3)=OIJ(I,J,IJ_NDETrhs3)+rhs_obio(i,j,10,10)
      OIJ(I,J,IJ_NDETrhs4)=OIJ(I,J,IJ_NDETrhs4)+rhs_obio(i,j,10,14)
      OIJ(I,J,IJ_NDETrhs5)=OIJ(I,J,IJ_NDETrhs5)+rhs_obio(i,j,10,16)
      ...
      OIJ(I,J,IJ_DICrhs6)=OIJ(I,J,IJ_DICrhs6)+rhs_obio(i,j,14,6)
#endif
...
!     .    obio_P(1,:),det(1,:),car(1,:)
       ...
       do k=1,kmax
        ...
        avgq(i,j,k)=avgq1d(k)
#ifdef OBIO_ON_GARYocean
!       if (vrbos) write(*,'(a,5i5,e12.4)')
!    .    'obio_model, avgq diag:',nstep,i,j,k,ijl_avgq,avgq1d(k)
...
!      if (p1d(kdm+1).gt.200.) then    !if total depth > 200m
     ...
     .       =pp2cocc_day(i,j)+pp2_1d(k,4)*24.d0  !mg,C/m2/day
              ...
              pp2tot_day(i,j)=pp2tot_day(i,j)+pp2_1d(k,nt)  !mg,Carbon/m2/hr
     .                                       * 24.d0        !->mg,Carbon/m2/day
          enddo
          enddo
!      endif
       if (vrbos) then
...
!           write(*,'(a,3i5,10e12.4)')'1111111111',
...
!    .      ,pp2diat_day_loc(i,j),pp2diat_dayav_loc(i,j)
!           endif
