...
!                     if(dts.eq.0)print*,'dts MK',k,j,dts,dt,Mk(k,j)
!     &                    ,nk(k),mtotal,dMdt(k,j)
     ...
     &                      ,nk(k),mtotal,dMdt(k,j),xk(k),xk(k+1)
                       ...
                       do kk=1,ibins
                         write(1044,*)'dts MK, Nk=',Nk(kk)
                       enddo
                       do jj=1,icomp-idiag
                         write(1044,*)'comp',jj
                         do kk=1,ibins
                         write(1044,*)'dts MK, Mk=',Mk(kk,jj)
                         enddo
                       enddo
                     endif
                  else
...
!       endif
      ...
      if(dts.eq.0) then
! When dts is zero due to the mass/number is out of size range, 
! mnfix is called. It might help to get non-zero dts next time.  
        Nki(:)=Nk(:)
        Mki(:,:)=Mk(:,:)
        ...
        if(count.lt.ibins) then
          print*,'dts=0 but mnfix worked',dts,tsum,count 
          Nk(:)=Nki(:)
          Mk(:,:)=Mki(:,:)
        endif
      endif
...
#if (defined TOMAS_30_10NM) || (defined TOMAS_30_1NM)
...
C     get some parameters
!      mu=2.5277e-7*temp**0.75302 
!      mfp=2.0*mu/(pres*sqrt(8.0*0.0289/(pi*R*temp)))  !S&P eqn 8.6 !bug?
!for old debugging      mfp=2.0*mu/(pres*sqrt(8.0*0.6589/(pi*gasc*temp)))  !S&P eqn 8.6 !bug?
...
c     Coefficients of total number of molecules in cluster     
     ...
     $         0.000610065, 0.000135751 /
      ...
      cna=cnai/5.               ! YUNHA  April, 2013 - Originally 2.d0-6 
...
!@auth   Jeff Pierce, April 2007
      ...
      parameter(A=2.0D-8)       ! YUNHA  April, 2013 - Originally 2.d0-6 
