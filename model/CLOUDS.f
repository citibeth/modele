...
!@var LDMIN the lowest layer the downdraft drops
     ...
     *     , MNdO,MNdL,MNdI,MCDNCW,MCDNCI,MRCLD   !Menon
#ifdef CLD_AER_CDNC
...
C**** CONDENSING TRACERS
 ...
 400   CONTINUE
     ...
     *     SULFINC,SULFOUT,TR_LEFT,TMP_SUL,TRCOND(1,L),AIRM,LHX,
...
c      IF(LHX.EQ.LHE) CDN3DL=SCDNCW
c      IF(LHX.EQ.LHE) CRE3DL=RCLDE
c      IF(LHX.EQ.LHS) CRE3DL=RCLDE
       PRS = (PL(1)-PTOP)/SIG(1)
...
c      IF (CLDSSL(L).GT.0.d0) then    !divide by CLDSSL for in-cld value
         IF (SVLHXL(L).EQ.LHE) CL3DL(L) = WMX(L)*RHODK*CD3DL(L) !/CLDSSL(L)   ! cld water kg m-2
         IF (SVLHXL(L).EQ.LHS) CI3DL(L) = WMX(L)*RHODK*CD3DL(L) !/CLDSSL(L)   ! ice water kg m-2
c      ENDIF
c      if(l.eq.1)
