...
c or Edgar-1995 surface sources
...
!@var SO2_src_3D SO2 volcanic, aircraft sources, biomass, Laki (kg/s)
...
!@var MDF is the mass of the downdraft flux
     ...
     * 6,7,7,8,9,9,10/)
      INTEGER, DIMENSION(10), PARAMETER :: LAKI_DAY = (/159,162,165,
     * 178,190,210,243,250,269,298/)
...
!@var A2D,B2D 2 dimensional fields to read in data
      ...
      idofm(2)=46               ! correct wrong value defined in MODEL_COM
...
c historic BC emissions
      ...
      integer iuc,ir,ihyr,i,j,id,jb1,jb2,iact,ii,jj,
     * iy,ip
...
c   then open new files
      ...
      call closeunit(iuc)
      hbc(:,:,1:2)=hbc_all(:,:,ir:ir+1)
      hoc(:,:,1:2)=hoc_all(:,:,ir:ir+1)
c kg/year to kg/s
      do id=1,2
      do i=1,im
      do j=1,jm
...
c historic SO2 emissions
      ...
      implicit none
      character*8 hname1,hname2
      integer iuc,ir,ihyr,i,j,id,jb1,jb2,iact
...
c     end do
c     end do
C Now put in separate subroutine so EDGAR-1995
C directive option can access the emission
...
!@auth Dorothy Koch
      USE TRACER_COM
      USE TRACER_DIAG_COM, only : tajls,taijs   
...
c     .                       + ( -trm(i,j,l,n)*(1.d0-d45)/dtsrc)
c     .                       + ( -trm(i,j,l,n)*(1.d0-d46)/dtsrc)
#else
