...
#ifdef NEW_IO_OCNINP
      allocate(dm(I_0H:I_1H,J_0H:J_1H))
#endif
...
!@auth M. Kelley
     ...
     &     tsi_top,tsnow_top,salt
      ...
      call read_dist_data(grid, fid, 'rsi', rsi)
      call read_dist_data(grid, fid, 'snow', snowi)
      call read_dist_data(grid, fid, 'msi', msi)
     ...
     &       (tsnow_top(i,j) - dtdz*.5d0*(zbot+ztop))
        enddo
        do l=1,2
          hsi(l,i,j) = Ei(tl(l),icefr(l)*salt(i,j))*ml(l)
          ssi(l,i,j) = (salt(i,j)*1d-3)*ml(l)*icefr(l)
        enddo
        do l=3,lmi
          hsi(l,i,j) = Ei(tl(l),salt(i,j))*ml(l)
          ssi(l,i,j) = (salt(i,j)*1d-3)*ml(l)
        enddo
