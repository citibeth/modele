#include "rundeck_opts.h"
      ...
      SUBROUTINE NITRATE_THERMO_DRV
!@sum
!@+     This routine sets up for and calls the thermodynamic module for aerosol
!@+     gas-particle partitioning.
!@+
!@+      A version of ISOROPIA 2 is the current thermodynamic model. 
!@auth Susanne Bauer
...
#ifdef TRACERS_TOMAS
      ASO4 = SUM(trm(i,j,l,IDTSO4:IDTSO4+nbins-1)) *1.d9 /AVOL
! TOMAS - Should I SKIP LAST TWO BINS (NO NITRATES FORMED IN COARSE MODE?)
      DUST = SUM(trm(i,j,l,IDTDUST:IDTDUST+nbins-1)) *1.d9 /AVOL
      SALT= SUM(trm(i,j,l,IDTNA:IDTNA+nbins-1)) *1.d9 /AVOL
#else
...
! save aerosol water (ug/m3)
      taijls(I,J,L,ijlt_aH2O)=taijls(I,J,L,ijlt_aH2O)+AH2O
! mol/m3 to mol/kg assuming density 1.d-3 kg/m3
      taijls(I,J,L,ijlt_apH)=taijls(I,J,L,ijlt_apH)+(-dlog10(AERLIQ(1)*1.d-3)) 
...
#ifdef TRACERS_TOMAS
!For TOMAS, NH3 and NH4 (src=1) are used for microphysics. 
! Ammonia residual
      tr3Dsource(i,j,l,nother,n_NH3)= ((GNH3 * 1.d-9 *AVOL) -trm(i,j,l,n_NH3)) /dtsrc
! Ammonium production
      tr3Dsource(i,j,l,nother,n_NH4)= ((ANH4 * 1.d-9 *AVOL) -trm(i,j,l,n_NH4)) /dtsrc
#else
! Ammonia residual
