...
c     use domain_decomp_1d, only : getDomainBounds, halo_update, 
      use domain_decomp_1d, only : get, halo_update, 
     *                             north, south
      USE OCEANR_DIM, only : grid=>ogrid
#ifdef OCN_GISS_TURB
...
c     write(30,*) "OBDRAG called .."
c     call getDomainBounds(grid, J_STRT=J_0, 
      call get(grid, J_STRT=J_0, 
     *     J_STRT_SKP=J_0S, J_STOP_SKP=J_1S,
     *     have_north_pole=have_north_pole)
C****
...
C****
      call halo_update (grid, mo, FROM=NORTH)
c     call halo_update (grid, uo, FROM=NORTH)
      call halo_update (grid, vo, FROM=SOUTH)
      call halo_update (grid, ut, FROM=SOUTH+NORTH)
      call halo_update (grid, vt, FROM=SOUTH)
#ifdef OCN_GISS_TURB
      call halo_update (grid, taubx, FROM=SOUTH+NORTH)
      call halo_update (grid, tauby, FROM=SOUTH)
      call halo_update (grid, rhobot, FROM=SOUTH+NORTH)
#endif
