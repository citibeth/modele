...
C**** dimensions IMIC = IM, JMIC = JM
      !!! hack for compilers which can't do shallow copy properly
      !!!TYPE(DIST_GRID), pointer :: grid_MIC => null()
...
C**** Sea ice velocity is northward at grid box edge
     ...
     *       -3d0*FASI(I,J))
      ...
      END IF
        AIJ(I,J,IJ_MVSI)=AIJ(I,J,IJ_MVSI)+SUM(FMSJ(I,1:LMI+2,J))
        AIJ(I,J,IJ_HVSI)=AIJ(I,J,IJ_HVSI)+SUM(FMSJ(I,3+LMI:4+2*LMI,J))
        AIJ(I,J,IJ_SVSI)=AIJ(I,J,IJ_SVSI)+SUM(FMSJ(I,5+2*LMI:6+3*LMI,J))
#ifdef TRACERS_WATER
         DO ITR=1,NTM
...
C**** Accumulate sea ice leaving and entering North Pole box
        ...
        SFMSI(1:NTRICE) = SFMSI(1:NTRICE) + FMSJ(I,1:NTRICE,JM-1)
         AIJ(I,JM-1,IJ_MVSI)=AIJ(I,JM-1,IJ_MVSI)
     *                        +SUM(FMSJ(I,1:LMI+2,JM-1))
         AIJ(I,JM-1,IJ_HVSI)=AIJ(I,JM-1,IJ_HVSI)
     &                        +SUM(FMSJ(I,3+LMI:4+2*LMI,JM-1))
         AIJ(I,JM-1,IJ_SVSI)=AIJ(I,JM-1,IJ_SVSI)+
     *      SUM(FMSJ(I,5+2*LMI:6+3*LMI,JM-1))
#ifdef TRACERS_WATER
           DO ITR=1,NTM
...
#ifdef NEW_IO
      USE DIAG_COM, only : make_timeaxis
#endif
