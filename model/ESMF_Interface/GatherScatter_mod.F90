...
!@sum For a given axisIndex array compute and initialize the cooresponding
!@+ array counts and displacements
...
!@sum This routine serves an an interface to the MPI routines that gather into specified 
!@+ locations from all processes in a group
...
!@sum This routine serves an an interface to the MPI routines that scatters a buffer in parts
!@+ to all processes in a communicator (as defined in the GRID).
    type (dist_grid) :: grid
...
!@sum Helper routine that returns a global MPI type
    integer, intent(in) :: baseType
    integer, intent(in) :: shp(:)
    integer, intent(in) :: dist_idx
...
!@sum This is a private generic type (handles integer, real, logical, etc...) gather routine
!@+ whose implementation details are left to the gather public interfaces
...
subroutine genericTypeScatter(grid, arr_glob, arr_loc, shp, numDims, dist_idx, &
     & baseType, baseSize)
!@sum This is a private generic type (handles integer, real, logical, etc...) scatter routine
!@+ whose implementation details are left to the scatter public interfaces
  use dist_grid_mod, only: dist_grid
