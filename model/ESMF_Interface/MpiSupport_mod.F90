...
!@sum Collection of low-level routines that support higher-level interfaces
...
!@var MpiType_type is a derived type to provide MPItype information
  type MpiType_type
    integer :: numBlocks
    integer :: blockLen
    integer :: stride
...
!@sum For a given rank, determine the rank's neighbors relative to the rank
!@+ Note these neighbors are only defined in the latitudinal direction
...
!@sum Returns a new MPI datatype from a given basetype
!@+ 
    ...
    ! A datatype has to be committed before it can be used...
...
!@sum Returns index of given MPI dtat type
...
!@sum Returns true if current rank is root process.
    integer :: rank
    integer :: ierr
...
!@sum Returns true if current communicator's rank is root process.
