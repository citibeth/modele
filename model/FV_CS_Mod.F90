...
module FV_CS_Mod
!@sum  This module contains procedures to support the modelE-FV interface
...
!@sum This routine creates an internal state (like a restart) with an
!@+ initialized dynamics state that is used by the FV-dynamical core
    USE DOMAIN_DECOMP_ATM, ONLY: GRID, GET, AM_I_ROOT, ESMF_GRID_ATM
...
!@sum Computes virtual pot. temp. from pot. temp. and specific humidity
...
!@sum Compute "pressure levels raised to the kappa" using GISS edge pressures
...
!@sum compute coefficients needed to calculate hybrid vertical coordinates
...
!@sum Write start date to a file using values from ESMF clock
      Type (ESMF_Clock), Intent(In) :: clock
      Integer          , Intent(In) :: unit
...
!@sum Initialize the ESMF framework and allocate FV-core export state
...
!@sum Update modelE's dynamics variables with FV core's exported state
...
!@sum Cubed-sphere modelE works with native-grid winds, so this routine is trivial.
...
!@sum Accumulate horizontal and vertical mass fluxes.
    use ESMFL_MOD, Only: ESMFL_StateGetPointerToData
    Use Resolution, only: LM
    USE GEOM, ONLY: AXYP
