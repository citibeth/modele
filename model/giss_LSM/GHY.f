...
!@var ae0 accumulated flux of total energy from atmosphere to surface (J/m^2)
      real*8, public :: aevapw,aevapd,aevapb,aepc,aepb,aepp,ae0,af1dt
...
C***
     ...
     &     ,aevapd,aevapw,ae0,af1dt,alhg,aruns,arunu,aflmlt,aintercep
...
!         endif
     ...
     &     .or. tp(1,1)<-150.d0 .or. tp(0,2)<-150.d0 )then
        write(6,*)'retp tp bounds error',tp(1,1),tp(0,2)
        write(6,*)'ijdebug',ijdebug
!        write(6,*)'tp(:,:): ',tp(:,:)
!        write(6,*)'ht(:,:): ',ht(:,:)        
!        write(6,*)'shc(:,:): ',shc
!        write(6,*)'w(:,:): ',w(:,:)
        write(6,*)'ibv  kk  tp  ht  shc  w fice'
        do kk=1,2  !LS_NFRAC-1
           do ibv=0,ngm
              if ((ibv<ng).and.(kk.le.2)) then
                 write(6,*) ibv,kk,tp(ibv,kk),ht(ibv,kk),shc(ibv,kk)
     &                ,w(ibv,kk),fice(ibv,kk)
              endif
           enddo
        enddo
        call reth
...
!     &       'retp: tground > 130C - see soil_outw and fort.99',255)
        write(6,*) 'WARNING: retp: tground > 130C - see soil_outw'
      endif
      return
      end subroutine retp
...
!-----------------------------------------------------------------------
     ...
     &     qm1_in,vs_in,vs0_in,gusti_in,tprime_in,qprime_in, ! forcing parameters
     &     end_of_day_flag
#ifdef TRACERS_WATER
     &     ,ghy_tr
#endif
...
cdddccc at the moment include update of vegetation structure 
cdddccc (for prognostic phenology) as a hack
cddd      if (end_of_day_flag) then
cddd        iu_debug = 800 + mod(ijdebug,100)
cddd        call ent_get_exports( entcell
cddd     &       ,C_entcell=C_before
cddd     &       )
cddd        !write(iu_debug,*) "before", ijdebug
cddd        !call ent_cell_print(iu_debug,entcell)
cddd        call ent_update_veg_structure(entcell)
cddd        call ent_get_exports( entcell
cddd     &       ,C_entcell=C_after
cddd     &       )
cddd        !write(iu_debug,*) "after"
cddd        !call ent_cell_print(811,entcell)
cddd        if ( abs(C_after-C_before) > 1.d-10 ) then
cddd          write(iu_debug,*) ijdebug, C_after-C_before, C_before
cddd        endif
cddd      endif
...
c       Qf=( evap_tot(2)/(rho/rhow*ch) + (vs-vs0)*qprime )/vs + qs
