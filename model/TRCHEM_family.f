...
c       for concentration of O:
        if (y(nO2,L) > 0.d0) then
          ...
          az=0.d0 ! this should be infinity, thus P1 should be zero below
        endif
c       for concentration of O(1D):
        bz=ss(2,L,I,J)/(rr(8,L)*y(nO2,L)+rr(9,L)*y(nM,L)+
     &  rr(10,L)*y(nH2O,L)+rr(11,L)*y(n_CH4,L))
        ! here we used to tune bz based at certain pressures
        if (y(nO2,L) > 0.d0) then
          P1=1.d0/(1.d0+az+bz)
        else
          P1=0.d0 ! az is infinity in this case
        endif
...
C**** GLOBAL parameters and variables:
      ...
      USE TRACER_COM, only         : n_NOx,n_Alkenes
...
!@var maxl LTROPO(I,J) or LS1-1, depending upon what_trop variable
      ...
      do L=1,lmax
        ! (here we used to set NO3 to zero at dawn)
...
c       Set NO3: D is loss rxns NO3->NO2 or NO
        D=ss(5,L,I,J)+ss(6,L,I,J)+rr(17,L)*p1*y(n_NOx,L)
     &    +rr(24,L)*p2*y(n_NOx,L)+rr(25,L)*yNO3(I,J,L)
     &    +rr(36,L)*y(n_Alkenes,L)

        yNO3(I,J,L)=((rr(7,L)*y(nO3,L)*p2*y(n_NOx,L))/D)
        if(yNO3(I,J,L).ge.1.d-1*y(n_NOx,L))yNO3(I,J,L)=1.d-1*y(n_NOx,L)

        y(nNO,L)= p1*(y(n_NOx,L)-yNO3(I,J,L))
        y(nNO2,L)=p2*(y(n_NOx,L)-yNO3(I,J,L))
...
C       Set limits on NO, NO2, NOx:
        ...
        pNO3(I,J,L)=yNO3(I,J,L)/y(n_NOx,L)
        y(nNO3,L) =yNO3(I,J,L)
        y(nHONO,L)=1.d0
      enddo
...
#else
       cqqz=(2.d0*(ss(4,L,I,J)*y(n_H2O2,L))+ss(9,L,I,J)*y(n_HNO3,L)+
     & 2.d0*(ss(13,L,I,J)*y(n_HCHO,L))+2.d0*ss(14,L,I,J)*
     & y(n_CH3OOH,L)+(rr(20,L)*y(nNO,L)+0.66d0*(rr(27,L)*
     & yCH3O2(I,J,L)))*yCH3O2(I,J,L))
#endif

       ! 1.66/1.31 accounts for HOx production via O(1D)+CH4-->CH3O path:
       cqqz=cqqz+
     & ((2.d0*rr(10,L)*y(nH2O,L)+(1.66d0/1.31d0)*rr(11,L)*y(n_CH4,L))*
     & y(nO1D,L))
...
c which also produces HO2 and R15 then S4/(S4+S14) fraction.
       ...
       if(cz+dz > 0.)then
         ...
         ! here we used to cap (limit) y(OH)
       else
         y(nOH,L)=1.d0
       endif
...
c C: prod equations in terms of HO2 (so *pHOx when OH is reactant):
     ...
     & +rr(84,L)*y(n_HBr,L)*y(nO,L)
       ...
       ! production from O1D no longer limited to O1D amount or O1D
       ! fraction via r10,11,s2:
