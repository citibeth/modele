...
!@sum The following module provides an interface for modelE to use the ESMF-wrapped version of
...
!@var FV_CORE_WRAPPER Derived type to encapsulate FV + modelE data
...
!-------------------------------------------------------------------------------
  subroutine init_esmf_clock
!-------------------------------------------------------------------------------
    use constant, only : hrday
    use MODEL_COM, only : DTsrc,itimei,itimee,nday,iyear1
    use ESMF
    implicit none
...
!      write(*,*)'Time Set End: ',ENDTIME
    ...
    call ESMF_timeintervalset(timeStep, S=INT(DTsrc), rc=rc)
  ...
  end subroutine init_esmf_clock

  subroutine run_fv(fv_wrapper)
!@sum Execute the FV method to integrate the core forward in time
...
!@sum Compute modelE's tendencies 
