...
cyhl      IF(NISUM .LE. 0.) print*, 'TOMASACT error', NSEC3,NPART,DPNEW
...
c DEC 2011/ YUNHA LEE - ND shouldn't be there? - Wei-Chun's paper have a type. ND is required. 
      THETA = (6D0/PI*LWC/DENW/ND*GK/GKP3)**(1D0/3D0)
c      THETA = (6D0/PI*LWC/DENW*GK/GKP3)**(1D0/3D0)
...
c  When does gamma function give the same values for different DB(I)/THETA?
      ...
      if(NTOT .EQ. 0. .and. I.EQ.30)THEN
!         PRINT*,'NTOT=0.',N0,ND,GINLARGE,GINSMALL
!         PRINT*,DB(I)/THETA,DB(I+1)/THETA,K
!         PRINT*,THETA,GK,GKP3
!	 PRINT*,LWMR,RHOSI
!         call stop_model('NTOT=0. will cause NBIN=INF',255)
        NTOT=100
      ENDIF
cyhl
      ENDDO
...
C
     ...
     &     IDTOCIL,IDTNUMD,IDTH2O,nbins
      implicit none 
      INTEGER KK,nseci,k,ICel,JCel,LCel,NMxi
...
C
      DO k=1,NSECi
         TPi(k) = TRM(ICel,JCel,LCel,IDTNUMD-1+k)   ! Number of particles (total)
...
C
         ELSE !should be changed to lower size bin. 
           Tt=Tt+Tpi(k)
!           PRINT*,'getCCN in',k,TRM(ICel,JCel,LCel,IDTNUMD-1+k),Tt,BOXVi
         endif

         Mso4   = TRM(ICel,JCel,LCel,IDTSO4-1+k) ! vhf is 2.5 - ammonium bisulfate
         SLFi   = SLFi + Mso4                       ! accumulate sulfate
         Mso4   = Mso4/(tr_mm(IDTSO4)*1e-3)/BOXVi   ! moles of SO4
         VhfSo4 = 2.5                               ! vhf is 2.5 - ammonium bisulfate
C
         Mna    = TRM(ICel,JCel,LCel,IDTNA-1+k)     ! vhf is 2.0
         Mna    = Mna/(tr_mm(IDTNA)*1e-3)/BOXVi     ! moles of NaCl
         VhfNacl= 2                                 ! vhf is 2 - sodium chloride

         Mocil    = TRM(ICel,JCel,LCel,IDTOCIL-1+k)     ! vhf is 2.0
         Mocil    = Mocil/(tr_mm(IDTOCIL)*1e-3)/BOXVi     ! moles of OCIL
