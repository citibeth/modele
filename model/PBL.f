...
!@var WS     = (=WSH) magn of GCM surf wind - ocean curr + buoyancy flux(m/s)
...
!$OMP  THREADPRIVATE (/PBLTPC/)
     ...
     *     USTAR,CM,CH,CQ,Z0M,Z0H,Z0Q,UG,VG,WG,W2_1,MDF,WINT,wsgcm,wspdf
...
C NBELL test code for biogenic emissions
#ifdef BIOGENIC_EMISSIONS
C emisop is in units kg C/m2/s
        select case (trname(ntix(itr)))
        case ('Isoprene')
        call ISOPRENE_EMISSION(ilong,jlat,itype,emisop)
        trcnst=emisop*byrho
