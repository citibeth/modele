...
!@calls RADPAR:RCOMP1, ORBPAR
     ...
     *     ,kradia,lm_req,pednl00,jyear,iyear1,itime,master_yr
      USE DOMAIN_DECOMP_ATM, only : grid, get, write_parallel, am_i_root
#ifndef CUBED_SPHERE
      USE GEOM, only : lat_dg,imaxj
#endif
      USE GEOM, only : lonlat_to_ij
!@var trrdry effective radius of radiatively active aerosol tracers (um).
!@+   For some reason this is not the same with the value used in TRACERS_DRV.f
!@+   which is used in sedimentation.
!@var wttr weighting array to weight main model tracers for radiation. The
...
!g95     *     ,FSXAER,FTXAER    ! scaling (on/off) for default aerosols
     ...
     *     ,wfac
      ...
      use tracers_dust, only : effRadMinerals
#endif
#ifdef TRACERS_AMP
      USE AERO_CONFIG, only: nmodes
      USE AMP_AEROSOL, only: AMP_DIAG_FC
#endif
      use AerParam_mod, only: dCDNC_est, aermix
      ...
      INTEGER L,LR,istart,n,nn,iu2 ! LONR,LATR
...
!@var QBIN true if files for radiation input files are binary
     ...
     *     ,.TRUE.,.TRUE.,.FALSE.,.TRUE.,.TRUE.,.TRUE.,.TRUE.,.TRUE./)
      ...
      integer :: nl,irec , iu_bio, lambda, lam(nlt)
      character title*50
...
!TOMAS does not include NO3 AND VOL, which use its default radiation. 
     ...
     &      +nraero_AMP+nraero_TOMAS+nraero_OM_SP
      ...
      if (.not.allocated(ttausv_nraero)) then
        allocate(ttausv_nraero(
     &         grid%i_strt_halo:grid%i_stop_halo,
     &         grid%j_strt_halo:grid%j_stop_halo,lm,nraero))
        ttausv_nraero = 0.
      endif
...
C     INPUT DATA  (i,j) dependent
     ...
     *             ,TRACER,FSTOPX,FTTOPX,chem_IN,O3natL,O3natLref
     &             ,nraero=>NTRACE
     *             ,FTAUC,LOC_CHL
     ...
     *             ,TAUWC_FIX_MET, TAUIC_FIX_MET
     *             ,TAUWC_TEMP, TAUIC_TEMP,SIZEWC_TEMP,SIZEIC_TEMP
     *             ,SIZEWC_FIX_MET,SIZEIC_FIX_MET
#endif
C     OUTPUT DATA
     ...
     *     ,cdncl,dALBsnX,rad_to_chem,trsurf
     *     ,FSRDIF,DIRNIR,DIFNIR,aer_rad_forc,clim_interact_chem
     *     ,TAUSUMW,TAUSUMI, WTRCLD, ICECLD, ijradfrc ! needed only if kradia<0
#ifdef ALTER_RADF_BY_LAT
     *     ,FULGAS_lat,FS8OPX_lat,FT8OPX_lat
#endif
#ifdef TRACERS_DUST
...
#endif
#ifdef CLD_ALB_FIX_MET
     &     ,TAUSUMW_FIX_MET,TAUSUMI_FIX_MET
#endif
      USE RANDOM
     ...
     *     ,ij_chl
     *     ,ij_swaerrf,ij_lwaerrf,ij_swaertrf,ij_lwaertrf
     *     ,ij_swaersrf,ij_lwaersrf
     *     ,ij_swaerrfnt,ij_lwaerrfnt,ij_swaertrfnt,ij_lwaertrfnt
     *     ,ij_swaersrfnt,ij_lwaersrfnt
...
#endif
#ifdef CLD_ALB_FIX_MET
     *     ,ij_swcrf_fix_met,ij_lwcrf_fix_met,ij_optdw_fix_met,
     *     ij_optdi_fix_met
#endif
     ...
     *     ,aiearth,fr_snow_rad_ij,fearth,snowd_ij=>snowd
#ifdef USE_ENT
...
!$OMP  THREADPRIVATE(/RADPAR_hybrid/)
     ...
     *     SNFSCRF,TNFSCRF,SNFSCRF2,TNFSCRF2
      REAL*8, DIMENSION(27,grid%I_STRT_HALO:grid%I_STOP_HALO,
     &                  grid%J_STRT_HALO:grid%J_STOP_HALO) ::
     *     SNFSAERRF,TNFSAERRF
#ifdef CLD_ALB_FIX_MET
      REAL*8, DIMENSION(grid%I_STRT_HALO:grid%I_STOP_HALO,
     ...
     *     SNFS_FIX_MET,TNFS_FIX_MET
      ...
      REAL*8, DIMENSION(grid%I_STRT_HALO:grid%I_STOP_HALO,
     &                  grid%J_STRT_HALO:grid%J_STOP_HALO) ::
     *     SNFS_BC_FIX_MET,ALB_BC_FIX_MET
#endif
...
!@var OLR_ij, OLR_cnt --  Net thermal radiation at TOA (W/m^2) for SUBDD
      ...
      REAL*8, dimension(grid%i_strt_halo:grid%i_stop_halo,
     &                  grid%j_strt_halo:grid%j_stop_halo) ::
     &     OLR_ij,SWU_ij,SDDARR,CTT_ij, CTP_ij, aaod_ij, aod_ij
     &     ,cldcv_ij
#ifdef TRACERS_ON
      real*8, dimension(grid%i_strt_halo:grid%i_stop_halo,
     &                  grid%j_strt_halo:grid%j_stop_halo,
     &                  lm,nraero) :: aod_tracer_ijl,aaod_tracer_ijl,
     &                  aod_cs_tracer_ijl,aaod_cs_tracer_ijl
#endif
      REAL*8, dimension(grid%i_strt_halo:grid%i_stop_halo,
     &                  grid%j_strt_halo:grid%j_stop_halo,lm) ::
     &     SDDARR3D, aaod_ijl, aod_ijl, cod_i_ijl, cod_w_ijl,
     &     aaod_cs_ijl,aod_cs_ijl
#endif
#ifdef ACCMIP_LIKE_DIAGS
...
!@+   3=LTROPO or LS1-1,auxiliary, 4=TOA,auxiliary; 5=LS1-1,reference
     ...
     *     ,onoff_chem,LFRC,JTIME,moddrf,nraero_internal
      REAL*8 ROT1,ROT2,PLAND,CSS,CMC,DEPTH,QSS,TAUSSL,RANDSS,TAUSSLIP
...
#else
      real*8 :: qcb_col(6,ICOMP-1)
#endif

#endif
      character(len=30) :: ntrace_str
      integer :: nntrace
...
!$OMP*   taugcb,dcc_cdncl,dod_cdncl,dCDNC,pvt0,
#ifdef ALTER_RADF_BY_LAT
...
!$OMP*   nrad,jtime,nday,
!$OMP*   idx,pedn,rhfix,nraero,ntrix,n_ocb, trm,n_ocii,n_ocia,
...
!$OMP*   FS8OPX,FT8OPX,SNFSAERRF,TNFSAERRF,diag_rad,
...
!$OMP*   ijts_tausub,ijlt_3dtau,ijlt_3daaod,ijts_sqexsub,ij_swncls,ij_lwdcls,
...
c      ILON72=INT(.5+(I-.5)*72./IM+.5)  ! lon_index w.r.to 72x46 grid
      ...
      FT8OPX(:)=FT8OPX_orig(:)*FT8OPX_lat(:,JLAT46)
        ...
        cod_i_ijl(i,j,:)= 0.d0
#ifdef TRACERS_ON
        aod_tracer_ijl(i,j,:,:)= 0.d0
        aaod_tracer_ijl(i,j,:,:)= 0.d0
        aod_cs_tracer_ijl(i,j,:,:)= 0.d0
        aaod_cs_tracer_ijl(i,j,:,:)= 0.d0
#endif
        aod_ijl(i,j,:)= 0.d0
        aaod_ijl(i,j,:)= 0.d0
        aod_cs_ijl(i,j,:)= 0.d0
        aaod_cs_ijl(i,j,:)= 0.d0
        aod_ij(i,j)= 0.d0
        aaod_ij(i,j)= 0.d0
#endif
      use_o3_ref = 0
...
C****        2 - keep "dimrad_sv" up-to-date:         dimrad_sv={
     ...
     *     ,wmag,snowoi,snowli,snowd !changed from snowe!1+1+1+1+
...
!****   find more derived fields and do some optional scalings
        ...
        sizewc = sizeic
#ifdef CLD_ALB_FIX_MET
        tauwc_fix_met= cldx*tauwc_fix_met
        tauic_fix_met= cldx*tauic_fix_met
        sizewc_fix_met= sizeic_fix_met
#endif
        if (kradia.gt.1) then
          do l=1,lm+lm_req
        ...
        COSZ=COSZA(I,J)
        zsnwoi=snowoi/rhos
        dALBsn = -dalbsnX*fsoot*depobc(i,j)
...
#ifdef CLD_ALB_FIX_MET
      OPTDW_FIX_MET=0. ; OPTDI_FIX_MET=0.
#endif
...
C**** save 3D cloud fraction as seen by radiation
          ...
          IF(TAUMCL.GT.TAUSSL+TAUSSLIP) THEN
...
#ifdef CLD_ALB_FIX_MET
            SIZEWC_FIX_MET(L)=CSIZMC_FIX_MET(L,I,J)
            SIZEIC_FIX_MET(L)=CSIZMC_FIX_MET(L,I,J)
#endif
            ...
            ELSE
              ...
              OPTDI=OPTDI+TAUIC(L)
#ifdef CLD_ALB_FIX_MET
              TAUIC_FIX_MET(L)=cldx*TAUMC_FIX_MET(L,I,J)
              OPTDI_FIX_MET=OPTDI_FIX_MET+TAUIC_FIX_MET(L)
#endif
          ...
          ELSE
...
#ifdef CLD_ALB_FIX_MET
            SIZEWC_FIX_MET(L)=CSIZSS_FIX_MET(L,I,J)
            SIZEIC_FIX_MET(L)=CSIZSS_FIX_MET(L,I,J)
#endif
            IF(SVLHX(L,I,J).EQ.LHE) THEN
              ...
              call inc_ajl(i,j,l,jl_wcldwt,pdsig(l,i,j))
#ifdef CLD_ALB_FIX_MET
              TAUWC_FIX_MET(L)=cldx*TAUSS_FIX_MET(L,I,J)
              OPTDW_FIX_MET=OPTDW_FIX_MET+TAUWC_FIX_MET(L)
#endif
            ...
            ELSE
              ...
              call inc_ajl(i,j,l,jl_icldwt,pdsig(l,i,j))
#ifdef CLD_ALB_FIX_MET
              TAUIC_FIX_MET(L)=cldx*TAUSS_FIX_MET(L,I,J)
              OPTDI_FIX_MET=OPTDI_FIX_MET+TAUIC_FIX_MET(L)
#endif
            END IF
          END IF
...
#ifdef CLD_ALB_FIX_MET
      if(optdw_fix_met.gt.0.) then
            AIJ(I,J,IJ_optdw_fix_met)=AIJ(I,J,IJ_optdw_fix_met)
     *       +optdw_fix_met
!            AIJ(I,J,IJ_wtrcld_fix_met)=AIJ(I,J,IJ_wtrcld_fix_met)+1.
!            WTRCLD_FIX_MET(I,J) =   1.
            TAUSUMW_FIX_MET(I,J) = OPTDW_FIX_MET
         end if
         if(optdi_fix_met.gt.0.) then
            AIJ(I,J,IJ_optdi_fix_met)=AIJ(I,J,IJ_optdi_fix_met)
     *          +optdi_fix_met
!            AIJ(I,J,IJ_icecld_fix_met)=AIJ(I,J,IJ_icecld_fix_met)+1.
!            ICECLD_FIX_MET(I,J) =  1.
            TAUSUMI_FIX_MET(I,J) = OPTDI_FIX_MET
         end if
#endif
...
C**** Only define TRACER is individual tracer is actually defined.
    ...
    (defined TRACERS_MINERALS) || (defined TRACERS_OM_SP)
...
C**** more than one tracer is lumped together for radiation purposes
      do n=1,nraero
        if (NTRIX(n).gt.0) then
          ...
          case ("OCIA")
           TRACER(L,n)=(trm(i,j,l,n_OCII)+trm(i,j,l,n_OCIA)
#ifdef TRACERS_AEROSOLS_OCEAN
     *                 +trm(i,j,l,n_ococean)
#endif  /* TRACERS_AEROSOLS_OCEAN */
     &                 )*BYAXYP(I,J)
...
#endif /* TRACERS_AEROSOLS_Koch/DUST/MINERALS/OM_SP */
...
c       JCKERR=JCKERR+1
     ...
     *                                .5d0*(PLB(LM+k)+PLB(LM+k+1)) )
        ...
        sizeic(LM+k)= 0.
#ifdef CLD_ALB_FIX_MET
        tauwc_fix_met(LM+k) = 0.
        tauic_fix_met(LM+k) = 0.
        sizewc_fix_met(LM+k)= 0.
        sizeic_fix_met(LM+k)= 0.
#endif
#ifdef TRACERS_ON
...
C**** Zenith angle and GROUND/SURFACE parameters
      ...
      SNOWD(:)=snowd_ij(:,I,J)                    ! snow depth (m)
...
C****          succeeds seasalt1 in nraero array
            ...
            CALL RCOMPX  ! tr.aero.Koch/dust/miner./om_sp
...
! use BC albedo from aerosol model 
        dalbsn=dalbsn1 
        CALL RCOMPX          ! cloud_rad_forc>0 : clr sky
        SNFS_BC_FIX_MET(I,J)=SRNFLB(LM+LM_REQ+1) ! always TOA
        ALB_BC_FIX_MET(I,J)=SRNFLB(1)/(SRDFLB(1)+1.D-20)

!reset to the model bc albedo 
        dALBsn=dALBsn_temp
#else
        dalbsn=0.d0
        CALL RCOMPX
...
C**** first, separate aerosols
          ...
          SNFSAERRF(N+18,I,J)=SRNFLB(LTROPO(I,J)) ! Tpause
          TNFSAERRF(N+18,I,J)=TRNFLB(LTROPO(I,J)) ! Tpause
...
C**** second, net aerosols
          ...
          SNFSAERRF(27,I,J)=SRNFLB(LTROPO(I,J)) ! Tpause
          TNFSAERRF(27,I,J)=TRNFLB(LTROPO(I,J)) ! Tpause
...
!     switch the cloud optical depth based on CDNC
        TAUWC_TEMP(:)=TAUWC(:)
        TAUIC_TEMP(:)=TAUIC(:) 
        SIZEWC_TEMP(:)=SIZEWC(:)
        SIZEIC_TEMP(:)=SIZEIC(:)
        ...
        TAUWC(:)=TAUWC_FIX_MET(:)
        TAUIC(:)=TAUIC_FIX_MET(:)
        SIZEWC(:)=SIZEWC_FIX_MET(:)
        SIZEIC(:)=SIZEIC_FIX_MET(:)
        ...
        SNFS_FIX_MET(I,J)=SRNFLB(LM+LM_REQ+1) ! always TOA
        TNFS_FIX_MET(I,J)=TRNFLB(LM+LM_REQ+1) ! always TOA

!     re-set the cloud optical depth that is not affected by CDNC
        TAUWC(:)=TAUWC_TEMP(:)
        TAUIC(:)=TAUIC_TEMP(:)
        SIZEWC(:)=SIZEWC_TEMP(:)
        SIZEIC(:)=SIZEIC_TEMP(:)
...
#ifdef TRACERS_AMP
#ifndef NO_HDIURN
      DIURN_AOT(I,J,1)   =  SUM(aesqex(1:Lm,6,1:nraero))
      DIURN_AOT(I,J,2)   =  SUM(aesqex(1:Lm,6,1:nraero))*OPNSKY
      DIURN_AOT(I,J,3)   =  SUM(aesqsc(1:Lm,6,1:nraero))
      DIURN_AOT(I,J,4)   =  SUM(aesqsc(1:Lm,6,1:nraero))*OPNSKY
#endif 
#endif
#ifdef TRACERS_TOMAS
#ifndef TRACERS_NITRATE
...
C**** Save optical depth diags
      ...
      do n=1,nraero
         if(ntrix(n) > 0) then 
#ifdef TRACERS_ON
      aod_tracer_ijl(i,j,1:lm,n)=aod_tracer_ijl(i,j,1:lm,n)+
     &                           aesqex(1:lm,6,n)
      aaod_tracer_ijl(i,j,1:lm,n)=aaod_tracer_ijl(i,j,1:lm,n)+
     &                            aesqex(1:Lm,6,n)-aesqsc(1:lm,6,n)
      aod_cs_tracer_ijl(i,j,1:lm,n)=aod_cs_tracer_ijl(i,j,1:lm,n)+
     &                              aesqex(1:lm,6,n)*OPNSKY
      aaod_cs_tracer_ijl(i,j,1:lm,n)=aaod_cs_tracer_ijl(i,j,1:lm,n)+
     &                       (aesqex(1:Lm,6,n)-aesqsc(1:lm,6,n))*OPNSKY
#endif
      aod_ijl(i,j,1:Lm)=aod_ijl(i,j,1:Lm) + (aesqex(1:Lm,6,n))
      aaod_ijl(i,j,1:Lm)=aaod_ijl(i,j,1:Lm)+(aesqex(1:Lm,6,n))
     &                                     -(aesqsc(1:Lm,6,n))
      aod_cs_ijl(i,j,1:Lm)=aod_cs_ijl(i,j,1:Lm)+
     &                     aesqex(1:Lm,6,n)*OPNSKY
      aaod_cs_ijl(i,j,1:Lm)=aaod_cs_ijl(i,j,1:Lm)+
     &                      (aesqex(1:Lm,6,n)-aesqsc(1:Lm,6,n))*OPNSKY
      aod_ij(i,j) = aod_ij(i,j) + SUM(aesqex(1:Lm,6,n))
      aaod_ij(i,j) = aaod_ij(i,j) + SUM((aesqex(1:Lm,6,n))
     &                                   -(aesqsc(1:Lm,6,n)))
         endif
      end do
#endif
     ...
     &           ,'ClayQuHe','ClayFeHe','ClayGyHe')
            nsub_ntrix(ntrix_i(n)) = nsub_ntrix(ntrix_i(n)) + 1
            IF (diag_rad /= 1) THEN
              IF ( ijts_tausub(1,ntrix_i(n),nsub_ntrix(ntrix_i(n)))>0)
     &             taijs(i,j,ijts_tausub(1,ntrix_i(n)
     &             ,nsub_ntrix(ntrix_i(n)))) = taijs(i,j,ijts_tausub(1
     &             ,ntrix_i(n),nsub_ntrix(ntrix_i(n)))) + SUM(ttausv
     &             (1:Lm,n))
              IF ( ijts_tausub(2,ntrix_i(n),nsub_ntrix(ntrix_i(n)))>0)
     &             taijs(i,j,ijts_tausub(2,ntrix_i(n)
     &             ,nsub_ntrix(ntrix_i(n)))) = taijs(i,j,ijts_tausub(2
     &             ,ntrix_i(n),nsub_ntrix(ntrix_i(n)))) + SUM(ttausv
     &             (1:Lm,n)) * OPNSKY
            END IF
     ...
     *           =taijls(i,j,1:lm,ijlt_3Dtau(NTRIX_I(n)))+TTAUSV(1:lm,n)
            IF (diag_rad == 1) THEN
              DO kr=1,6
               IF ( ijts_sqexsub(1,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n))) 
     &              > 0 ) taijs(i,j,ijts_sqexsub(1,kr,ntrix_i(n)
     &              ,nsub_ntrix(ntrix_i(n)))) = taijs(i,j,ijts_sqexsub(1
     &              ,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n))))+SUM(aesqex
     &              (1:Lm,kr,n))
               IF ( ijts_sqexsub(2,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n))) 
     &              > 0 ) taijs(i,j,ijts_sqexsub(2,kr,ntrix_i(n)
     &              ,nsub_ntrix(ntrix_i(n)))) = taijs(i,j,ijts_sqexsub(2
     &              ,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n)))) +
     &              SUM(aesqex(1:Lm,kr,n)) * OPNSKY
               IF ( ijts_sqscsub(1,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n))) 
     &              > 0 ) taijs(i,j,ijts_sqscsub(1,kr,ntrix_i(n)
     &              ,nsub_ntrix(ntrix_i(n)))) = taijs(i,j,ijts_sqscsub(1
     &              ,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n)))) +
     &              SUM(aesqsc(1:Lm,kr,n))
               IF ( ijts_sqscsub(2,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n))) 
     &              > 0 ) taijs(i,j,ijts_sqscsub(2,kr,ntrix_i(n)
     &              ,nsub_ntrix(ntrix_i(n)))) = taijs(i,j,ijts_sqscsub(2
     &              ,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n)))) +
     &              SUM(aesqsc(1:Lm,kr,n)) * OPNSKY
               IF ( ijts_sqcbsub(1,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n))) 
     &              > 0 ) taijs(i,j,ijts_sqcbsub(1,kr,ntrix_i(n)
     &              ,nsub_ntrix(ntrix_i(n)))) = taijs(i,j,ijts_sqcbsub(1
     &              ,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n)))) +
     &              SUM(aesqcb(1:Lm,kr,n)) / (SUM(aesqsc(1:Lm,kr,n))
     &               +1.D-10)
               IF (ijts_sqcbsub(2,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n))) 
     &              > 0) taijs(i,j,ijts_sqcbsub(2,kr,ntrix_i(n)
     &              ,nsub_ntrix(ntrix_i(n)))) = taijs(i,j,ijts_sqcbsub(2
     &              ,kr,ntrix_i(n),nsub_ntrix(ntrix_i(n)))) +
     &              SUM(aesqcb(1:Lm,kr,n)) / (SUM(aesqsc(1:Lm,kr,n))
     &              +1.D-10) * OPNSKY
              END DO
     ...
     *            (aesqex(1:lm,6,n)-aesqsc(1:lm,6,n))
     ...
     &           =taijls(i,j,1:lm,ijlt_3Dtau(NTRIX_I(n)))+TTAUSV(1:lm,n)
            ...
            IF (diag_rad == 1) THEN
              DO kr=1,6
c           if (kr == 1 ) print*,'SUSA ',SUM(aesqex(1:Lm,kr,n)),kr,n
...
#ifdef CACHED_SUBDD
         CTT_ij(i,j) = (tlb(ltopcl+1) - tf)
         CTP_ij(i,j) = plb(ltopcl+1)
#endif
...
C**** Save cloud tau=1 related diagnostics here (opt.depth=1 level)
      ...
      DO L=LM,1,-1
         ...
         taudn=tauup+taucl
#ifdef CACHED_SUBDD
         cod_w_ijl(i,j,l) = tauwc(l)
         cod_i_ijl(i,j,l) = tauic(l)
#endif
         if (taudn.gt.1.) then
...
C YUNHA - FOR NOW, SNFSCRF is the ones without aerosols.
           AIJ(I,J,IJ_SWCRF_FIX_MET)=AIJ(I,J,IJ_SWCRF_FIX_MET)+
     +          (SNFS_FIX_MET(I,J)-SNFSCRF(I,J) )*CSZ2
           AIJ(I,J,IJ_LWCRF_FIX_MET)=AIJ(I,J,IJ_LWCRF_FIX_MET)-
...
C**** AERRF diags if required
     ...
     *            (TNFS(3,I,J)-TNFSAERRF(N,I,J))
             AIJ(I,J,IJ_SWAERTRF+N-1)=AIJ(I,J,IJ_SWAERTRF+N-1)+
     *            (SNFS(4,I,J)-SNFSAERRF(N+18,I,J))*CSZ2
             AIJ(I,J,IJ_LWAERTRF+N-1)=AIJ(I,J,IJ_LWAERTRF+N-1)-
     *            (TNFS(4,I,J)-TNFSAERRF(N+18,I,J))
     ...
     *          (TNFS(3,I,J)-TNFSAERRF(17,I,J))
           AIJ(I,J,IJ_SWAERTRFNT)=AIJ(I,J,IJ_SWAERTRFNT)+
     *          (SNFS(4,I,J)-SNFSAERRF(27,I,J))*CSZ2
           AIJ(I,J,IJ_LWAERTRFNT)=AIJ(I,J,IJ_LWAERTRFNT)-
     *          (TNFS(4,I,J)-TNFSAERRF(27,I,J))
...
C**** define SNFS/TNFS level (TOA/TROPO) for calculating forcing
     ...
     *   + 100.d0*(ALBNBC(I,J)-ALB(I,J,1))
     ...
     &         +(SNFS(3,I,J)-NFSNBC(I,J))*CSZ2
      ...
      if (ijts_alb(1).gt.0)
     * TAIJS(I,J,ijts_alb(1))=TAIJS(I,J,ijts_alb(1))
     *   + 100.d0*(ALBNBC(I,J)-ALB_BC_FIX_MET(I,J))
      if (ijts_alb(2).gt.0)
     & taijs(i,j,ijts_alb(2))
     &     =taijs(i,j,ijts_alb(2))
...
c     ..........
     ...
     &                ,'ClayIlHe','ClayKaHe','ClaySmHe' ,'ClayCaHe'
     &                ,'ClayQuHe','ClayFeHe','ClayGyHe')
                 nsub_ntrix(ntrix(n)) = nsub_ntrix(ntrix(n)) + 1
c shortwave forcing (TOA or TROPO) of Clay sub size classes
                 if (ijts_fcsub(1,ntrix(n),nsub_ntrix(ntrix(n))) > 0)
     &                taijs(i,j,ijts_fcsub(1,ntrix(n)
     &                ,nsub_ntrix(ntrix(n)))) = taijs(i,j,ijts_fcsub(1
     &                ,ntrix(n),nsub_ntrix(ntrix(n)))) + rsign_aer
     &                *(snfst(2,n,i,j)-snfs(lfrc,i,j))*csz2
c longwave forcing  (TOA or TROPO) of Clay size sub classes
                 if (ijts_fcsub(2,ntrix(n),nsub_ntrix(ntrix(n))) > 0)
     &                taijs(i,j,ijts_fcsub(2,ntrix(n)
     &                ,nsub_ntrix(ntrix(n)))) = taijs(i,j,ijts_fcsub(2
     &                ,ntrix(n),nsub_ntrix(ntrix(n)))) - rsign_aer
     &                *(tnfst(2,n,i,j)-tnfs(lfrc,i,j))
c shortwave forcing (TOA or TROPO) clear sky of Clay sub size classes
                 if (ijts_fcsub(5,ntrix(n),nsub_ntrix(ntrix(n))) > 0)
     &                taijs(i,j,ijts_fcsub(5,ntrix(n)
     &                ,nsub_ntrix(ntrix(n)))) = taijs(i,j,ijts_fcsub(5
     &                ,ntrix(n),nsub_ntrix(ntrix(n)))) + rsign_aer
     &                *(snfst(2,n,i,j)-snfs(lfrc,i,j))*csz2 * (1.D0
     &                -cfrac(i,j))
c longwave forcing  (TOA or TROPO) clear sky of Clay sub size classes
                 if (ijts_fcsub(6,ntrix(n),nsub_ntrix(ntrix(n))) > 0)
     &                taijs(i,j,ijts_fcsub(6,ntrix(n)
     &                ,nsub_ntrix(ntrix(n)))) = taijs(i,j,ijts_fcsub(6
     &                ,ntrix(n),nsub_ntrix(ntrix(n)))) - rsign_aer
     &                *(tnfst(2,n,i,j)-tnfs(lfrc,i,j)) * (1.D0-cfrac(i
     &                ,j))
c shortwave forcing at surface (if required) of Clay sub size classes
                 if (ijts_fcsub(3,ntrix(n),nsub_ntrix(ntrix(n))) > 0)
     &                taijs(i,j,ijts_fcsub(3,ntrix(n)
     &                ,nsub_ntrix(ntrix(n)))) = taijs(i,j,ijts_fcsub(3
     &                ,ntrix(n),nsub_ntrix(ntrix(n)))) + rsign_aer
     &                *(snfst(1,n,i,j)-snfs(1,i,j))*csz2
c longwave forcing at surface (if required) of Clay sub size classes
                 if (ijts_fcsub(4,ntrix(n),nsub_ntrix(ntrix(n))) > 0)
     &                taijs(i,j,ijts_fcsub(4,ntrix(n)
     &                ,nsub_ntrix(ntrix(n)))) = taijs(i,j,ijts_fcsub(4
     &                ,ntrix(n),nsub_ntrix(ntrix(n)))) - rsign_aer
     &                *(tnfst(1,n,i,j)-tnfs(1,i,j))
c shortwave forcing at surface clear sky (if required) of Clay sub size classes
                 if (ijts_fcsub(7,ntrix(n),nsub_ntrix(ntrix(n))) > 0)
     &                taijs(i,j,ijts_fcsub(7,ntrix(n)
     &                ,nsub_ntrix(ntrix(n)))) = taijs(i,j,ijts_fcsub(7
     &                ,ntrix(n),nsub_ntrix(ntrix(n)))) +rsign_aer
     &                *(snfst(1,n,i,j)-snfs(1,i,j))*csz2 * (1.D0-cfrac(i
     &                ,j))
c longwave forcing at surface clear sky (if required) of Clay sub size classes
                 if (ijts_fcsub(8,ntrix(n),nsub_ntrix(ntrix(n))) > 0)
     &                taijs(i,j,ijts_fcsub(8,ntrix(n)
     &                ,nsub_ntrix(ntrix(n)))) = taijs(i,j,ijts_fcsub(8
     &                ,ntrix(n),nsub_ntrix(ntrix(n)))) - rsign_aer
...
c
      ...
      case ('ctp')
        call inc_subdd(subdd,k,CTP_ij)
      case ('ctt')
        call inc_subdd(subdd,k,CTT_ij)
      case ('cldcv')
        call inc_subdd(subdd,k,cldcv_ij)
      case ('aod')
        call inc_subdd(subdd,k,aod_ij)
      case ('aaod')
        call inc_subdd(subdd,k,aaod_ij)
      end select
...
C**** consideration is needed, speak with M.Kelly
      ...
      select case (subdd%name(k))
         ...
         case ('lwhr')
            ...
            call inc_subdd(subdd,k,sddarr3d)
      case ('aod_3d')
        call inc_subdd(subdd,k,aod_ijl)
      case ('aaod_3d')
        call inc_subdd(subdd,k,aaod_ijl)
      case ('aod_cs_3d')
        call inc_subdd(subdd,k,aod_cs_ijl)
      case ('aaod_cs_3d')
        call inc_subdd(subdd,k,aaod_cs_ijl)
      case ('cod_w_3d')
        call inc_subdd(subdd,k,cod_w_ijl)
      case ('cod_i_3d')
        call inc_subdd(subdd,k,cod_i_ijl)
         end select
      enddo
      ...
      nntrace=0
      do n=1,nraero
        if (ntrix(n).eq.0) cycle
        if (trim(trname(ntrix(n))).eq.'Clay') then
          nntrace=nntrace+1
          write(ntrace_str,'("'//trim(trname(ntrix(n)))//'",i1)')nntrace
        else
          ntrace_str=trim(trname(ntrix(n)))
        endif
        do k=1,subdd%ndiags
          if('aod_'//trim(ntrace_str)//'_3d'.eq.
     &       trim(subdd%name(k))) then
            do L=1,LmaxSUBDD
              sddarr3d(:,:,L) = aod_tracer_ijl(:,:,L,n)
            enddo
            call inc_subdd(subdd,k,sddarr3d)
          endif
          if('aaod_'//trim(ntrace_str)//'_3d'.eq.
     &       trim(subdd%name(k))) then
            do L=1,LmaxSUBDD
              sddarr3d(:,:,L) = aaod_tracer_ijl(:,:,L,n)
            enddo
            call inc_subdd(subdd,k,sddarr3d)
          endif
          if('aod_cs_'//trim(ntrace_str)//'_3d'.eq.
     &       trim(subdd%name(k))) then
            do L=1,LmaxSUBDD
              sddarr3d(:,:,L) = aod_cs_tracer_ijl(:,:,L,n)
            enddo
            call inc_subdd(subdd,k,sddarr3d)
          endif
          if('aaod_cs_'//trim(ntrace_str)//'_3d'.eq.
     &       trim(subdd%name(k))) then
            do L=1,LmaxSUBDD
              sddarr3d(:,:,L) = aaod_cs_tracer_ijl(:,:,L,n)
            enddo
            call inc_subdd(subdd,k,sddarr3d)
          endif
        enddo
