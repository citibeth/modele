...
c  Initialize
      ...
      tracer(:,:,:,1)=fldo2         !because 1 is nintrate
...
! otherwise take from rundeck
      ...
      enddo
      enddo
      !alkalinity is in umol/kg
      tracer(:,:,:,ntyp+n_inert+ndet+ncar+1)=alk       !because ntrac is alkalinity
#ifdef TOPAZ_params
      tracer(:,:,:,ntyp+n_inert+ndet+ncar+2)=0.          !Ca_det=0 initially
#endif
#endif
...
#ifdef TRACERS_Alkalinity
           !alkalinity
           nt = ntyp+n_inert+ndet+ncar+1
           tracer(i,j,k,nt) = alk(i,j,k)

#ifdef TOPAZ_params
           !ca_det_calc
           nt = ntyp+n_inert+ndet+ncar+2
           tracer(i,j,k,nt) = 0.
#endif
#endif
...
c    conversion from uM to mg/m3
      ...
      do k=1,kdm
          !convert to uM
          tracer(i,j,k,ntyp+n_inert+ndet+2) =
     .       1024.5 * 0.001 *  dicmod(i,j,k)
