...
!@var PIfact strat-overwrite adjustment for preindustrial runs
      ...
      REAL*8 fact_so4
      REAL*8, DIMENSION(IM,JM,LM) :: NO2MASS
      REAL*8, DIMENSION(IM,JM) :: NO2_COL,NO2_TOT
...
#ifdef SHINDELL_STRAT_CHEM
      if(H2ObyCH4.ne.0.)               
     &call stop_model('H2ObyCH4.ne.0 and strat chem is on.',255)
#endif
#ifdef INTERACTIVE_WETLANDS_CH4
C not really related to chemistry, but convenient place to do this:
C**** update running-averages for interactive wetlands CH4:
      do J=1,JM; do I=1,IMAXJ(J)
        temp_SW=SALBFJ(I,J)*(SRDN(I,J)+1.d-20)*COSZ1(I,J)
...
C info to set strat H2O based on tropical tropopause H2O and CH4:
      if(Itime.eq.ItimeI)then
        ...
        countTT=0
        do J=1,JM
          if(LAT_DG(J,1).ge.-20..and.LAT_DG(J,1).le.20.)then
            do I=1,IM
              avgTT_H2O = avgTT_H2O + Q(I,J,LTROPO(I,J))*MWabyMWw
              avgTT_CH4 = avgTT_CH4 + trm(I,J,LTROPO(I,J),n_CH4)
     &        *mass2vol(n_CH4)*BYDXYP(J)*BYAM(LTROPO(I,J),I,J)
        ...
        if(countTT.le.0)
     &  call stop_model('problem with countTT in masterchem',255)
        avgTT_H2O=avgTT_H2O/real(countTT)
        avgTT_CH4=avgTT_CH4/real(countTT)
      end if
#endif
...
!$OMP* no2mass,no2_col,no2_tot,
#ifdef SHINDELL_STRAT_CHEM
!$OMP* CLTOT, ClOx_old, COLMO2, COLMO3, changeClONO2, changeClOx,
...
#ifdef SHINDELL_STRAT_CHEM
         call stop_model
     &   ('fixed ch4 chemi in stratosphere needs attention.',255)
#endif
         if(J.lt.JEQ)then ! SH
...
#ifdef SHINDELL_STRAT_CHEM
c init. strat H2O & Q!, based on tropical tropopause H2O and CH4:
       if(Itime.eq.ItimeI .and. L.gt.LTROPO(I,J)) then
         y(nH2O,L) = 
     &   y(nM,L)*(avgTT_H2O + 2.d0*(avgTT_CH4-y(n_CH4,L)/y(nM,L)))
...
c         SF3 is photolysis of water in Schumann-Runge bands
c          based on Nicolet, Pl. Space Sci., p 871, 1983.
          if(pres2(L).le.10.)then
          ...
          SF3(I,J,L)=SF3(I,J,L)*
     &    by35*SQRT(1.224d3*(cos(ABS(LAT_DG(J,1))*radian))**2.+1.d0)
C         SF2 is photlysis of NO in bands (0-0) and (1-0)
c          based on Nicolet, Pl. Space Sci., p 111, 1980
          if(L.gt.maxl)then
           IF(colmO2.GT.2.d19)then
...
c
         ...
         if(wprod_sulf.ge.0.5d0*y(n_N2O5,L))wprod_sulf=0.5d0*y(n_N2O5,L)
c        define O3 from Ox:
         y(nO3,L)=y(n_Ox,L)*pOx(I,J,L)
c
c        calculate change in NO3:
...
cc     Het rxn ClONO2+H2O on sulfate (and PSCs if present)
         if(rr(106,L).gt.2.d-35)then
          changehetClONO2=-(rr(106,L)*y(n_ClONO2,L))*dt2
          if(changehetClONO2.ge.0.5*y(n_ClONO2,L))changehetClONO2=
     &     -0.5d0*y(n_ClONO2,L)
...
c 109 N2O5    +HCl     -->Cl      +HNO3        !really makes ClNO2  2
         ...
         if((lat_dg(J,1).LE.-66.).OR.(lat_dg(J,1).GE.66.))then
         if(ta(l).le.T_thresh)then
           pscX=1.d0
...
c     back to the gas phase
         if(pres2(L).lt.245.d0 .and. pres2(L).ge.31.6d0 .and.
     &   (J.le.JSS+1.or.J.ge.JNN+1) .and. ta(L).le.T_thresh)then
           changeHNO3 = changeHNO3 - 0.01d0*y(n_HNO3,L)
         endif
         if(aero(L) .ne. 0)   
     &   changeHNO3 = changeHNO3 - 1.1d-4*y(n_HNO3,L)
C
C Make sure nighttime chemistry changes are not too big:
C
         error=.false.
...
C NO2 column diagnostic for comparison with satellite products
C Accumulate at 10.30AM local time only
C nbell
c initialise
        NO2_COL(I,J)=0.0d0
        NO2_TOT(I,J)=0.0d0

       if( abs( mod(ndaynb+itime+.501 + 
     &   ((i-.5*(im+1))*ndaynb)/im ,ndaynb ) - 10.5*ndaynb/24)
     &  .lt.0.5 ) then

        DO L=1,maxl
C Convert to kg NO2 in grid box
        NO2MASS(I,J,L)=(y(nNO2,L)/y(nM,L))
     &    *DXYP(J)*AM(L,I,J)*(46.d0/29.d0)
C Sum the column
        NO2_TOT(I,J)=NO2_TOT(I,J) + NO2MASS(I,J,L)
        ENDDO
C Convert to molecule/cm2 
       NO2_COL(I,J)=(NO2_TOT(I,J)/46.0d-3)*avog
     &   *1.0d-4*BYDXYP(J)
C Accumulate diagnostic
        TAIJS(I,J,ijs_NO2col)=TAIJS(I,J,ijs_NO2col)+NO2_COL(I,J)      
...
Corig     rmv=(1.d0-0.95d0**(maxl-L)) ! had level dependence
     ...
     &        ))           ! about 5% at 100mb and 60% at 1000mb
        ...
        END IF
CCC     IF (L.gt.maxl) changeL(L,n_HCHO)=-trm(I,J,L,n_HCHO)*0.1d0
c
c Set CLTOT based on CFCs (3.3 ppbv yield from complete oxidation of
c 1.8 ppbv CFC plus 0.5 ppbv background)
C
      if(L.gt.maxl)then
C       warning: resetting some y's here, so don't reuse them later     
...
c
c     Also set total bromine based on CFCs
...
c
c Set Bromine (using CLTOT name) based on CFCs (1.0 pptv yield from 
c  complete oxidation of 1.8 ppbv CFC plus 0.5 pptv background)
...
C
C     if(checktracer_on) call checktracer(I,J)
c
...
cc    Some General Overwriting of Tracers               cc
cc  (which get charged to "stratospheric overwriting")  cc
cc(also sets tr3Dsource(nChemistry,n)=0, if overwriting)cc
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C If fix_CH4_chemistry is turned on (=1) then reset the CH4 tracer to
C initial conditions (mixing ratio units!) in whole atmosphere:
...
C      
#ifdef SHINDELL_STRAT_CHEM
      if(PI_run.gt.0) call stop_model(
     & 'masterchem: some overwriting for PI runs attention',255)
C For N2O and CFC, we have L=1 overwriting:
      do j=1,jm
       do i=1,IMAXJ(j)
        tr3Dsource(i,j,1,nChemistry,n_N2O)=0.d0
        tr3Dsource(i,j,1,nStratwrite,n_N2O)=(290.d-9*tr_mm(n_N2O)
     &  *bymair*am(1,i,j)*dxyp(j) -trm(i,j,1,n_N2O))*bydtsrc
        tr3Dsource(i,j,1,nChemistry,n_CFC)=0.d0
        tr3Dsource(i,j,1,nStratwrite,n_CFC)=(1722.d-12*tr_mm(n_CFC)
     &  *bymair*am(1,i,j)*dxyp(j) -trm(i,j,1,n_CFC))*bydtsrc
       end do
      end do
C
C For     NOx, BrOx, and ClOx, we have overwriting where P < 0.1mb:
C For Ox,                      we have overwriting where P < 0.5mb:
C     Interpolate BrOx & ClOx altitude-dependence to model resolution:
      CALL LOGPINT(LCOalt,PCOalt,BrOxaltIN,LM,PRES2,BrOxalt,.true.)
      CALL LOGPINT(LCOalt,PCOalt,ClOxaltIN,LM,PRES2,ClOxalt,.true.)
      do L=LS1,LM
       if(pres2(L).lt.0.5)then
        do j=1,jm         
        do i=1,IMAXJ(j)
         tr3Dsource(i,j,L,nChemistry,n_Ox)=0.d0
         if(correct_strat_Ox) then
           tr3Dsource(i,j,L,nStratwrite,n_Ox)=(O3_rad_save(L,i,j)*
     &     dxyp(j)*O3MULT*corrOx(J,L,imonth)-trm(i,j,L,n_Ox))*bydtsrc
         else
           tr3Dsource(i,j,L,nStratwrite,n_Ox)=(O3_rad_save(L,i,j)*
     &     dxyp(j)*O3MULT                   -trm(i,j,L,n_Ox))*bydtsrc
         end if        
CCCCCCCCCC This section is here hopefully temporarily, CCCCCCCCCCCCCC
CCCCCCCCCC to prevent a high polar temperature from    CCCCCCCCCCCCCC
CCCCCCCCCC      crashing the stratospheric drag.       CCCCCCCCCCCCCC
C         if(j.ge.JM-1 .and. L.ge.LM-1)then
C           if((trm(I,J,L,n_Ox)+ 
C     &     tr3Dsource(i,j,l,nChemistry,n_Ox)*dtsrc + 
C     &     tr3Dsource(i,j,L,nStratwrite,n_Ox)*dtsrc)*mass2vol(n_Ox)*
C     &     BYDXYP(j)*BYAM(L,I,J)*1.d6 .gt. 0.7d0) then
C           write(666,666)I,J,L, (trm(I,J,L,n_Ox)+
C     &     tr3Dsource(i,j,l,nChemistry,n_Ox)*dtsrc +
C     &     tr3Dsource(i,j,L,nStratwrite,n_Ox)*dtsrc)*mass2vol(n_Ox)*
C     &     BYDXYP(j)*BYAM(L,I,J)*1.d6
C           tr3Dsource(i,j,l,nChemistry,n_Ox) =0.d0
C           tr3Dsource(i,j,L,nStratwrite,n_Ox)=(0.7d-6*dxyp(j)*
C     &     am(L,I,J)*bymass2vol(n_Ox)-trm(i,j,L,n_Ox))*bydtsrc
C 666       format
C     &     ('master: IJL=',3(1X,I2),' trm limited to 0.7 from',F10.3)
C           end if
C         end if
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     &    -trm(i,j,L,n_BrOx))*bydtsrc
         tr3Dsource(i,j,L,nChemistry,n_NOx)=0.d0
         tr3Dsource(i,j,L,nStratwrite,n_NOx)=(75.d-11 ! 75=1*300*2.5*.1
     &    *am(L,i,j)*dxyp(j) -trm(i,j,L,n_NOx))*bydtsrc
        end do ! I
        end do ! J
       end if
      end do
...
C
     ...
     & byam(L569M,:,J)+F569P*trm(:,J,L569P,n_CH4)*byam(L569P,:,J))
     & /IMAXJ(J)
      END DO
...
c
          if (fix_CH4_chemistry.eq.0) then ! ------
c
...
C**** GLOBAL parameters and variables:
C
      USE MODEL_COM, only: LM,JM,LS1,JEQ
#ifdef SHINDELL_STRAT_CHEM
     &     ,ptop,psf,sig,Itime,ItimeI
      USE RADNCB, only : AX_rad_save
...
C
      ...
      if(AX_rad_save(1,I,J).ne.0.)call stop_model('kext prob 0',255)
      do L=2,Ltop
        if(AX_rad_save(L,I,J).ne.0..and.AX_rad_save(L-1,I,J).eq.0.)
     &   LAXb=L
        if(AX_rad_save(L,I,J).eq.0..and.AX_rad_save(L-1,I,J).ne.0.)
     &   LAXt=L-1
      end do
#else
      select case(which_trop)
...
C       if(l.lt.12.or.l.gt.18)then   !aerosols (14-33 km) & PSCs 14-22 km
        if(pres(l).ge.245.d0 .or. pres(l).le.5.d0)then 
...
cc        (in km**-1) from SAGE II data on GISS web site
          if(pres(l).lt.245.d0.and.pres(l).gt.150.d0)rkext(l)=0.d0
          if(pres(l).le.150..and.pres(l).gt.31.60)then
            if(l.lt.LAXb) then
              rkext(l)=5.d-2*AX_rad_save(LAXb,i,j)
            else if(l.gt.LAXt) then
              rkext(l)=0.33d0*rkext(l-1)
            else
              rkext(l)=5.d-2*AX_rad_save(l,i,j)
            endif
          endif
          ...
          if(pres(l).le.10.0d0.and.pres(l).ge.4.6d0)then
            if(l.lt.LAXb) then
              ...
              rkext(l)=4.d0*6.6667d-1*rkext(l-1)
            else
              rkext(l)=5.d-2*AX_rad_save(l,i,j)
            endif
          endif

          IF(PRES(L).lt.90. .and. J.GT.JS .and. J.LE.JN)
     &      rkext(l)=rkext(l)*0.1d0 !<<<<<<<<<<< NOTE <<<<<<<<<<<
    
          if(rkext(l).ne.0.)aero(l) = 1

          if(Itime.eq.ItimeI.and.i.eq.50.and.j.eq.38)write(6,668)
     &    L,PRES(L),AX_rad_save(l,i,j),rkext(l),LAXb,LAXt
  668     format('L=',I2,' P=',E10.4,' RAD=',E10.4,' kext=',E10.4,2I3)

cc        PSCs (pressure < 245mb and pressure > 5mb here)
          pscEx(l)=0.d0!NAT PSC surface conc per unit volume (cm^2/cm^3)
          if(pres(l).ge.31.6d0.and.ta(l).le.T_thresh)then
            if((lat_dg(J,1).LE.-66.).OR.(lat_dg(J,1).GE.66.)) then
              pscEx(l)=1.0d-8
              if(ta(l).le.T_thresh-7.d0)pscEx(l)=pscEx(l)*1.d1
            endif
          endif
...
c         Reaction 2 on sulfate and PSCs      
          ...
          rr(nr2+nr3+2,L)=0.5d0*rkext(l)*1.d-5*temp*4.d-3
          if(pres(l).gt.31.6d0) rr(nr2+nr3+2,L)=
     &    rr(nr2+nr3+2,L)+0.25d0*pscEx(l)*temp*0.004d0
