...
!#endif
     ...
     &      chemrate(iBrOplusNO2,lprn) > 0.5d0*y(n_BrOx,lprn))then
              write(out_line,110)
...
c     In the stratosphere, calculate ozone change due to rxn with atomic H:
      ...
      do L=maxT+1,LM
        if(OxlossbyH(L)<y(n_Ox,L))dest(n_Ox,L)=dest(n_Ox,L)-OxlossbyH(L)
        if(prnchg.and.J==jprn.and.I==iprn) then ! .and.l==lprn) then
...
! here we USED TO remove some of the HNO3 formed heterogeneously, as it doesn't
! come back to the gas phase.
