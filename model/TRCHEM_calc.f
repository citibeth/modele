...
C
     ...
     &                            ,LAT_DG,LON_DG
#endif
      USE TRACER_DIAG_COM, only : jls_OHcon,jls_day,tajls,
...
c      HCHO, Alkenes, and CO per rxn, correct here following Houweling
      ...
      enddo
c
c#ifdef SHINDELL_STRAT_CHEM
c     modify Cl source from CFC photolysis to account for other
c      sources (e.g. other CFCs, HCFCs, methyl chloride)
c      do L=maxT+1,LM
c       prod(n_ClOx,L)=prod(n_ClOx,L)+ss(26,L,I,J)*y(n_CFC,L)*dt2*0.75d0
c     add Br source using CFC photolysis as proxy
c       prod(n_HBr,L)=prod(n_HBr,L)+ss(26,L,I,J)*y(n_CFC,L)*dt2*1.d-3
c      enddo
c
c     Remove some lower strat polar HNO3 in PSCs
c     do L=1,maxL
c      IF(PRES(L).lt.245.d0 .and. PRES(L).ge.31.6d0)THEN
c        if(ta(L).lt.198.d0)then
C this was:if(J.le.17.or.J.ge.39)dest(n_HNO3,L)=
c          if(J.le.JS+1.or.J.ge.JNN+1)dest(n_HNO3,L)=
c    *     dest(n_HNO3,L)-0.2d0*y(n_HNO3,L)
c        endif
c      END IF
c     enddo
c#endif
...
c        increase product gases
     ...
     &  +rr(82,l)*y(nCl,L)
       ...
       tempiter=rr(20,L)*y(nNO,L)+rr(22,L)*y(nHO2,L)
     & +rr(85,l)*y(nClO,l)
 15    CH3O2loss=tempiter+rr(27,L)*yCH3O2(I,J,L)
       if(CH3O2loss.gt.1d-7)then
          y(nCH3O2,L)=qqqCH3O2/CH3O2loss
        else
          y(nCH3O2,L)=1.d-5
        ...
        iter=iter+1
        if(iter.le.7)goto15 ! replace with while loop?
c       Conserve carbon wrt CH3O2 changes
        diffCH3O2=y(nCH3O2,L)-yCH3O2(I,J,L)
        if(diffCH3O2.gt.0.d0)then
c        reduce source gases (CH4 and CH3OOH)
...
c      Calculate water vapor change
     ...
     *   (rr(11,L)*y(nO1D,L)+rr(12,L)*y(nOH,L)+rr(82,L)*y(nCl,L)))*dt2
       enddo
c
c     Calculate ozone change due to within NOx partitioning
      do L=1,LM
       if(y(nO1D,L).eq.0.) CYCLE
...
c
     ...
     *    ,ss(24,lprn,i,j)*y(n_HOBr,lprn)*dt2
         write(6,110)'loss by rxn 73 removed'
     *    ,chemrate(73,lprn)
         endif
     ...
     *    ,ss(23,lprn,i,j)*y(n_BrONO2,lprn)*dt2
         write(6,110)'loss by rxn 104 removed'
     *    ,chemrate(104,lprn)
         endif
        endif
        if(igas.eq.n_NOx)then
     ...
     *    ,chemrate(55,lprn)
         write(6,110)'loss by rxn 63 removed'
     *    ,chemrate(63,lprn)
         endif
     ...
     *    ,chemrate(65,lprn)
         write(6,110)'loss by rxn 103 removed'
     *    ,chemrate(103,lprn)
         endif
        endif
        if(igas.eq.n_NOx)then
...
c        Cacluate Cl2 amount to P/L:
         if((ss(18,L,I,J)+rr(51,L)*y(nOH,L)).gt.0)then
...
c          reduce N production to match NOx loss
     ...
     *      changeL(L,n_BrONO2)*mass2vol(n_BrONO2)
           ...
           if (sumP.eq.0) print*, 'SUMP = 0***', sumP, L,i,j,
     *                 changeL(L,n_HNO3)*mass2vol(n_HNO3)
           ratioP=newP/sumP
     ...
     *      (mass2vol(n_BrONO2)/mass2vol(n_BrOx)) !ensure Br cons
          ...
          if (ratio.le.1.d0.and.ratio.gt.0.d0)then
c          reduce NOx destruction to match N production (3/24/2004)
           changeL(L,n_NOx)=changeL(L,n_NOx)*ratio
#ifdef SHINDELL_STRAT_CHEM
           changeL(L,n_NOx)=changeL(L,n_NOx)-
     *      2.d0*sv_changeN2O(L)*mass2vol(n_N2O)/mass2vol(n_NOx)
...
c      changeL(L,n_N_d1)=changeL(L,n_N_d1)-(krate(i,j,l,1,1)
c     & *y(n_HNO3,l)*dt2)*rMAbyM(L)*dxyp(J)/mass2vol(n_HNO3)
c      changeL(L,n_N_d2)=changeL(L,n_N_d2)-(krate(i,j,l,1,2)
c     & *y(n_N2O5,l)*dt2)*rMAbyM(L)*dxyp(J)/mass2vol(n_N2O5)
c      changeL(L,n_N_d3)=changeL(L,n_N_d3)-(krate(i,j,l,1,3)
c     & *y(nNO3,l)*dt2)*rMAbyM(L)*dxyp(J)/mass2vol(nNO3)
...
#ifdef SHINDELL_STRAT_CHEM
      if(L.gt.maxT)then
c     Calculate NOx and Ox changes due to molecular nitrogen
c      produced by SRB photlysis (SF2 is NO + hv rate)
       byta=1.d0/ta(L)
c       rxnN1=3.8d-11*exp(85d0*byta)*y(nOH,L)    ! N+OH->NO+H, not in JPL (rates from IUPAC 1989)
       rxnN2=1.5d-11*exp(-3600.d0*byta)*y(nO2,L) ! N+O2->NO+O
       rxnN3=5.8d-12*exp(220.d0*byta)*y(nNO2,L)  ! N+O2->N2O+O
       rxnN4=2.1d-11*exp(100d0*byta)*y(nNO,L)    ! N+O2->N2+O
       NprodOx=2.d0*SF2(I,J,L)*y(nNO,L)*dt2
       NlossNOx=NprodOx*(rxnN3+rxnN4)/(rxnN2+rxnN3+rxnN4)
       changeL(L,n_NOx)=changeL(L,n_NOx)-NlossNOx
     *  *(dxyp(J)*rMAbyM(L))/(mass2vol(n_NOx))
       changeL(L,n_Ox)=changeL(L,n_Ox)+NprodOx
     *  *(dxyp(J)*rMAbyM(L))/(mass2vol(n_Ox))
       if(prnchg.and.J.eq.jprn.and.I.eq.iprn.and.l.eq.lprn)
     * write(6,*) 'NOx loss & Ox gain due to rxns  w/ N '
     & ,NlossNOx,NprodOx
      endif
#endif
      end do ! L
...
c     back to the gas phase
      do L=1,maxL
       IF(PRES(L).lt.245.d0.and.PRES(L).ge.31.6d0 .and.
     & (J.le.JSS+1.or.J.ge.JNN+1) .and. ta(L).le.T_thresh)
...
c         if(L.le.maxT.and.igas.eq.n_BrOx.or.igas.eq.n_BrONO2)
c     *    changeL(L,igas)=0.d0
c         if(L.le.maxT.and.igas.eq.n_HBr.or.igas.eq.n_HOBr)
c     *    changeL(L,igas)=0.d0
c         if(PRES(L).gt.300.d0.and.igas.eq.n_HOCl)changeL(L,igas)=0.d0
c#endif
       end do    ! L
      end do     ! igas
