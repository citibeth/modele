...
c
     ...
     &     ,ndyno,dto,dts
      ...
      use odiag, only : koijl,igrid_oijl,jgrid_oijl,sname_oijl
     &     ,oijl_out,oijl=>oijl_loc
     ...
     &     ,oij=>oij_loc,ij_sf,olnst,ln_mflx,ijl_isdm
     ...
     &     ,pack_data,unpack_data ! for horz stream function
      ...
      real*8 mass,gos,sos,temgs,volgs,volgsp,fac,facst,wdenom,xedge,
     &     ffac,
     &     wtdn,wtup,massdn,massup,dpr
...
c****
c**** derive bolus vertical mass flux from horizontal mass fluxes
c**** for skew-GM
        do l=1,lmo-1
        do j=j_0s,j_1s
          i=1
...
c
     ...
     &       -dmfwb(i,j,l)*ffac ! subtract bolus-induced part of remap flux
        oijl_out(i,j,l,ijl_mfwb) = oijl(i,j,l,ijl_mfwb)/dxypo(j)
     ...
     &       +dmfwb(i,j,l)*xedge !      add bolus-induced part of remap flux
...
#ifdef TDMIX_AUX_DIAGS
        oijl_out(i,j,l,ijl_gsymmf+2) = oijl(i,j,l,ijl_gsymmf+2)/dxypo(j)
        oijl_out(i,j,l,ijl_ssymmf+2) = oijl(i,j,l,ijl_ssymmf+2)/dxypo(j)
#endif
        ...
        if(use_tdiss==1) then
          oijl_out(i,j,l,ijl_kvx) = oijl(i,j,l,ijl_kvx)
        endif
...
c
     ...
     &         'oijl_prep: OCEAN_TENDENCY_DIAGS name mismatch',255)
      ...
      do k=1,kk
        ...
        do l=1,lmo; do j=j_0s,j_1s
          mfwb(:,j,l) = mfwb(:,j,l)*dxypo(j)
        enddo; enddo
        call do_fluxconv_3d(
     &       oijl(:,:,:,flxind(k)+0),
     &       oijl(:,:,:,flxind(k)+1),
