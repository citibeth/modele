...
!@ver  2015/03/13
     ...
     *     ,lmu,lmv,hatmo,hocean,mo,g0m,gxmo,gymo,gzmo
     *     ,s0m,sxmo,symo,szmo,uo,vo,uod,vod,dxypo,ogeoz
     *     ,OWALLU,OWALLV
     *     ,dts,dtolf,dto,dtofs,nocean,mdyno,msgso
     *     ,ndyno,imaxj,ogeoz_sv,bydts,lmo_min,j1o
     *     ,OBottom_drag,OCoastal_drag,OTIDE,XOWALL,VCOAST,oc_salt_mean
      USE OCEAN, only : ze,dzoe,bydzoe,zmid
...
#endif
c**** Extract domain decomposition info
      INTEGER :: J_0, J_1, J_0S, J_1S, J_0H, J_1H, J1H,JNH
      LOGICAL :: HAVE_NORTH_POLE
      CALL GET(grid, J_STRT = J_0, J_STOP = J_1
     *      ,J_STRT_SKP  = J_0S, J_STOP_SKP  = J_1S
     *      ,J_STRT_HALO  = J_0H, J_STOP_HALO  = J_1H
     *      ,HAVE_NORTH_POLE = HAVE_NORTH_POLE)
      J1H = Max (GRID%J_STRT - 1, 1)  ;  JNH = Min (GRID%J_STOP + 1, JM)
...
C****
      ...
      Call SYNC_PARAM ("XOWALL",XOWALL)
      Call SYNC_PARAM ("VCOAST",VCOAST)
...
C**** Read in table function for specific volume
      ...
      If (AM_I_ROOT())  Write (6,*) 'Read unit',IU_OFTAB,': ',TITLE
...
C**** Fix to 4 for temporary consistency
      J1O=4
      If (AM_I_ROOT())  Write (6,*) 'Fixed J1O:',J1O
...
!**** Compute ocean wall arrays OWALLU,OWALLV,OWALLST if necessary
      If (OBOTTOM_DRAG == 2)  Then
         Allocate (OWALLU(IM,J1H:JNH,LMO),OWALLV(IM,J1H:JNH,LMO),
     *             OWALL4(IM,JM))
!**** Convert east-west OWALL4 (10^-6/m) to OWALLU*XOWALL (1/m)
         Call OPENUNIT ("OWALL", IU_OWALL, .True., .True.)       
         Do 210 L=1,LMO
         Read  (IU_OWALL, Err=821) TITLE,OWALL4
         If (AM_I_ROOT())  Write (6,*) 'OWALL: Read',IU_OWALL,': ',TITLE
         Do 210 J=J1H,JNH  ;  Do 210 I=1,IM
         OWALLU(I,J,L) = OWALL4(I,J) * XOWALL * .875d-6
  210    OWALLV(I,J,L) = OWALL4(I,J) * XOWALL * .125d-6
!**** Convert north-south OWALL4 (10^-6/m) to OWALLV*XOWALL (1/m)
         Do 220 L=1,LMO
         Read  (IU_OWALL, Err=821) TITLE,OWALL4
         If (AM_I_ROOT())  Write (6,*) 'OWALL: Read',IU_OWALL,': ',TITLE
         Do 220 J=J1H,JNH  ;  Do 220 I=1,IM
         OWALLV(I,J,L) = OWALLV(I,J,L) + OWALL4(I,J) * XOWALL * .875d-6
  220    OWALLU(I,J,L) = OWALLU(I,J,L) + OWALL4(I,J) * XOWALL * .125d-6
         Call CLOSEUNIT (IU_OWALL)
         Deallocate (OWALL4)
!**** Check for missing WALL data in ocean grid cells
         Do 230 J=J1H,JNH  ;  Do 230 I=1,IM  ;  Do 230 L=1,LMM(I,J)
         If (OWALLU(I,J,L) < 0)  Then
            Write (6,*) 'OWALL: Missing WALL data, set to 0:',I,J,L 
            OWALLU(I,J,L) = 0  ;  OWALLV(I,J,L) = 0  ;  EndIf
  230    Continue  ;  EndIf
...
#ifdef TEMPSALT_OIC
      call tempsalt_oic(mo,g0m,gzmo,s0m,szmo)
      gzmo = 0.
      szmo = 0.
      ! copy distributed real*8 arrays back into the global-domain
      ! real*4 arrays to avoid having to rewrite or duplicate
      ! various bits of coding below
...
C???? For starters, let all processes read the IC
      ...
      If (AM_I_ROOT())  Write (6,*) 'OIC read unit',IU_OIC,': ',TITLE
...
C**** Allocate(/read) arrays for KPP mixing scheme
...
#ifdef  apply_FreshWaterAnomaly
      call init_stream(grid,FAFMIP_WFOstream,'FAFMIP_WFO',
     &     'wfo',-1d30,1d30,'ppm',
...
C**** Terminate because of improper start up
  ...
  821 Call STOP_MODEL ('Error reading OWALL file',255)
C****
...
C****
      ...
      subroutine tempsalt_oic(mo,g0,gz,s0,sz)
...
#ifdef  apply_WindStressAnomaly
      use timestream_mod, only : init_stream,read_stream
      use model_com, only : jyear,jday
      use ocean, only : im
      use oceanr_dim, only : grid=>ogrid
      use ocean, only : FAFMIP_TAUU_stream,FAFMIP_TAUV_stream
      use ocean, only : FAFMIP_tauu,FAFMIP_tauv
#endif
...
#ifdef  apply_WindStressAnomaly
      if(.not.allocated(FAFMIP_tauu)) then
C**** initialize streams if starting up
        call init_stream(grid,FAFMIP_TAUU_stream,
     &       'FAFMIP_TAUU','tauu',
     &       -1d30,1d30,'linm2m',jyear,jday)
        call init_stream(grid,FAFMIP_TAUV_stream,
     &       'FAFMIP_TAUV','tauv',
     &       -1d30,1d30,'linm2m',jyear,jday)
        allocate(FAFMIP_tauu(im,grid%j_strt_halo:grid%j_stop_halo))
        allocate(FAFMIP_tauv(im,grid%j_strt_halo:grid%j_stop_halo))
      endif
C**** time-interpolate to current year and day
      call read_stream(grid,FAFMIP_TAUU_stream,jyear,jday,FAFMIP_tauu)
      call read_stream(grid,FAFMIP_TAUV_stream,jyear,jday,FAFMIP_tauv)
#endif
...
!#endif
#endif
#ifdef  apply_FreshWaterAnomaly
      use ocean, only : FAFMIP_WFOstream,dts
      use model_com, only : jyear,jday
     ...
     *     ,DS0(LMO),GF0,GFREZS,TFREZS,TEMGSP,SHCGS,GF00
     ...
     &     fafmip_wfo  ! kg/m2/s water flux, positive downward

      call read_stream(ogrid,FAFMIP_WFOstream,jyear,jday,fafmip_wfo)
#endif
...
#ifdef  apply_FreshWaterAnomaly
        ! M.K. 11/5/2015
        ! Apply FAFMIP perturbation water flux.  Since the temperature
        ! of the fluxed water is not specified, it is assumed equal to
        ! the local SST. This does not conserve energy, but avoids
        ! potential artifacts where imposed perturbations are large
        ! (river mouths, sea ice formation/melt).  The salinity of
        ! the fluxed water is taken to be zero, which in ice-covered
        ! areas is not exactly consistent with ice formation/melt physics.
        dmo1 = fafmip_wfo(i,j)*dts
        g0ml(1) = g0ml(1)*(1d0 + dmo1/mo1)
        mo1 = mo1 + dmo1

        !if(i.eq.1 .and. j.eq.90 .and. mod(itime,48).eq.0) then
        !  write(6,*) 'fafmip_wfo ',jday,fafmip_wfo(i,j)
        !endif
#endif
