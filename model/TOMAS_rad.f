...
C ************************************************************                          
     ...
     &     ,TOMAS_qabs ! aerosol radiative properties from lookup table
...
!      if (itime.ne.itimeI) then 
      ...
      do K=1,NBINS 
        ...
        do c=1,icomp-2
          ...
          mh2o=mh2o+m_spec(c)*(wrmass-1.d0)  !aerosol-water mass[kg]
...
!Negligible number and mass ==> zero AOD
     ...
     &       .or.mtot.le.0.) goto 500
        ...
        if(mp.gt.1000.*xk(nbins+1).or.mp.lt.xk(1)/10.)then 
          print*,'mp is out of range',mp,k,i,j,l,rhe,mtot,mh2o,
     &         trm(i,j,l,idtnumd-1+k),m_spec 
!In AR5_v2_branch, this print-out alot because ECOB mass is somehow quite in higher layer. I think this is related to CLOUDS. 
        endif
        ...
        do c=1,icomp-2
          ...
          if(abs(reffwet).le.0.or. abs(density(c)).le.0.) then
            reffwet=5.0e-10 
            density(c)=1500. 
            m_spec(c)=0. !set zero 
          endif
...
!              if (AM_I_ROOT()) 
CCC (IMPORTANT) In AR5_v2_branch, so many print out of large x1. 
CCC   I think this is related to CLOUDS (maybe advection in CLOUDS?). 
CCC   When turning off the CONDSE in ModelE, no more printout. 
CCC   This wasn't the case for AR5_aer. 

             ! print*, 'x1 is too big', x1,reffwet*1e6,i,j,l,k
CCC (IMPORTANT) END
              x1=1500.
            endif
