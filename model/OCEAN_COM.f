C****   
C**** OCEAN_COM.f    Module Variables for Ocean    2015/03/13
C****
...
#endif
#if defined(apply_WindStressAnomaly) || defined(apply_FreshWaterAnomaly)
...
!@dbparam OBOTTOM_DRAG = 0: no velocity drag on ocean bottom
!@+ (def) OBOTTOM_DRAG = 1: OBDRA2 apply velcocity drag on lowest layer
!@+       OBOTTOM_DRAG = 2: OWALL slows velocity by sea mount walls 
!@dbparam OCOASTAL_DRAG = 0: no drag on tracer gradients at coast lines
!@+ (def) OCOASTAL_DRAG = 1: OCOAST reduces tracer gradients at coasts
!@+       OCOASTAL_DRAG = 2 and OBOTTOM_DRAG = 2:
!@+          OWALL reduces perpendicular tracer gradients by sea mounts
!@dbparam OTIDE = 1: Lunar,Solar tides accelerate UO,VO,UOD,VOD, MUST 
      Integer*4 ::
     *     OBottom_drag = 1,    
     *     OCoastal_drag = 1,
     *     OTIDE = 0
!@dbparam XOWALL: dVO = - VO * dT * OWALL*XOWALL * |WO|
!@dpparam VCOAST: dGXM = - GXM * dT * OWALL*XOWALL * VCOAST
      Real*8 :: XOWALL = .25, VCOAST = .0625
...
!@var G0M specific pot. enthalpy G multiplied by gridbox mass (J)
!@var S0M salinity S multiplied by gridbox mass (kg)
!@var OWALLU = east-west blocking wall (10^-6/m)
!@var OWALLV = north-south blocking wall (10^-6/m)
      REAL*8, ALLOCATABLE, DIMENSION(:,:,:):: MO,UO,VO,UOD,VOD,
     *     G0M,S0M, OWALLU,OWALLV

!@var USE_QUS whether to use second-order moments for G, S, tracers
!@+   (0 if not, 1 if yes)
      INTEGER :: USE_QUS=0
!@var [G,S][X,Y,Z]MO
!@+      gradients of G, S mult. by grid mass (J, kg)
!@var [G,S][XX,YY,ZZ,XY,YZ,ZX]MO
!@+      2nd-order moments of G, S mult. by grid mass (J, kg)
...
!@var T[X,Y,Z]MO
!@+       gradients of tracer mult. by grid mass (kg)
!@var T[XX,YY,ZZ,XY,YZ,ZX]MO
!@+       2nd-order moments of tracer mult. by grid mass (kg)
      REAL*8, ALLOCATABLE, DIMENSION(:,:,:,:) ::
     *     TRMO, TXMO,TYMO,TZMO, TXXMO,TYYMO,TZZMO, TXYMO,TYZMO,TZXMO
#endif
...
!@var remap_a2o,remap_o2a atm->ocn,ocn->atm interpolation info
     ...
     *      UXA,UXB,UXC,UYA,UYB,UYC,VXA,VXB,VXC,VYA,VYB,VYC
      ...
      type(timestream) :: FAFMIP_TAUU_stream,FAFMIP_TAUV_stream
      real*8, dimension(:,:), allocatable :: FAFMIP_tauu,FAFMIP_tauv
#endif
#ifdef  apply_FreshWaterAnomaly
      type(timestream) :: FAFMIP_WFOstream
#endif
...
!@ver  1.0
     ...
     *       ,TXXMO,TYYMO,TZZMO,TXYMO,TYZMO,TZXMO
     *       ,TRMO_glob,TXMO_glob,TYMO_glob,TZMO_glob
      USE OCN_TRACER_COM, only : ntm
#endif
