...
C**** regime (but not above P_CSDRAG)
     ...
     &     dua_tmp,dva_tmp,dud_tmp,dvd_tmp
...
c
      if(ang_sdrag.eq.1) then
        ...
        do i=i_0,i_1
          ...
          dud_tmp(i,j,l) = du(i,j)
          dvd_tmp(i,j,l) = dv(i,j)
        enddo
        enddo
        enddo
      endif

      call interp_dgrid_to_agrid(dud_tmp, dvd_tmp, dua_tmp, dva_tmp,
     &     rotate=.true.)

      do l=1,lm
      do j=j_0,j_1
      do i=i_0,i_1
        call inc_ajl(i,j,l,jl_dudtsdrg,dua_tmp(i,j,l))
      enddo
      enddo
      enddo
...
c
     ...
     &        +(MU_INC(3)*UR(3)+MU_INC(3+7)*UR(3+7))*(WT(3))
      AIJ(I,J,IJ_GW5)=AIJ(I,J,IJ_GW5)
     &        +(MU_INC(7)*UR(7)+MU_INC(7+7)*UR(7+7))*(WT(7))
      ...
      DO N=1,9 ! 9 is without additional convective waves
        ...
        if(n.ge.3 .and. n.le.8) then ! additional convective waves
          DO L=1,LM
            call inc_ajl(i,j,l,N+JL_gwFirst-1,DUGWD(L,N+7))
          END DO
        endif
      END DO 
