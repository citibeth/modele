...
c
      ...
      SSMAO=SSMAL+SSM2+SSM9                           ! Land aerosols (SSMAL) + Sea-salt
c
      IF(SSMAL.le.100.d0) SSMAL=100.d0
      IF(SSMAO.le.50.d0) SSMAO=50.d0
...
c
C  TOMAS (December, 2011) has too high CDNC from this method. 
c$$$      DO K=1,NBINS
...
c$$$!YUNHA LEE(10/12/2011) - Model crash at this point due to mtot(k)=0.0?  
c$$$!            So "if statement" is added to prevent any floating error. 
c$$$        if(mtot(k).gt.0.)then
