...
c these routines are only needed when running on multiple CPUs
      use dd2d_utils, only : pack_row,unpack_row,get_nlnk,
...
c these routines are only needed when running on multiple CPUs
      ...
      interface par_read_jdecomp_optimized
        module procedure par_read_ij
        module procedure par_read_ijx
        module procedure par_read_ijxx
        module procedure par_read_ijxxx
      end interface
#endif /* not SERIAL_MODE */

      interface alloc_global
        module procedure alloc_global1
        module procedure alloc_global2
        module procedure alloc_global3
        module procedure alloc_global4
        module procedure alloc_global5
      end interface
...
c
      ...
      real*8, pointer :: arrgij(:,:)
      integer :: rc
      call alloc_global(grid,arrgij,grid%npx,grid%npy)
     ...
     &       rc = nf_put_vara_double(fid,vid,srt,cnt,arrgij)
      ...
      real*8 :: arr(:,:,:,:)
      integer :: fid,vid,jdim
      real*8, pointer :: arrgxij(:,:,:)
      integer :: rc,k,srt(4),cnt(4)
      call alloc_global(grid,arrgxij,size(arr,1),grid%npx,grid%npy)
     ...
     &       rc = nf_put_vara_double(fid,vid,srt,cnt,arrgxij)
      ...
      subroutine par_read_ij(grid,fid,vid,arr,jdim)
      type(dist_grid), intent(in) :: grid
      real*8 :: arr(:,:)
      integer :: fid,vid,jdim
      real*8, pointer :: arrgij(:,:)
      ...
      call alloc_global(grid,arrgij,grid%npx,grid%npy)
      if(grid%am_i_globalroot) then
        rc = nf_get_var_double(fid,vid,arrgij)
      endif
      call unpack_data(grid,arrgij,arr)
      deallocate(arrgij)
      return
      end subroutine par_read_ij
      subroutine par_read_ijx(grid,fid,vid,arr,jdim)
      type(dist_grid), intent(in) :: grid
      real*8 :: arr(:,:,:)
      integer :: fid,vid,jdim
      real*8, pointer :: arrgijk(:,:,:)
      integer :: rc
      call alloc_global(grid,arrgijk,grid%npx,grid%npy,size(arr,3))
      if(grid%am_i_globalroot) then
        rc = nf_get_var_double(fid,vid,arrgijk)
      endif
      call unpack_data(grid,arrgijk,arr)
      deallocate(arrgijk)
      return
      end subroutine par_read_ijx
      subroutine par_read_ijxx(grid,fid,vid,arr,jdim)
      type(dist_grid), intent(in) :: grid
      real*8 :: arr(:,:,:,:)
      integer :: fid,vid,jdim
      real*8, pointer :: arrgijkl(:,:,:,:)
      integer :: rc
      if(jdim.eq.3) then
        call par_read_xijx(grid,fid,vid,arr,jdim)
        return
      endif
      call alloc_global(grid,arrgijkl,grid%npx,grid%npy,
     &     size(arr,3),size(arr,4))
      if(grid%am_i_globalroot) then
        rc = nf_get_var_double(fid,vid,arrgijkl)
      endif
      call unpack_data(grid,arrgijkl,arr)
      deallocate(arrgijkl)
      return
      end subroutine par_read_ijxx
      subroutine par_read_ijxxx(grid,fid,vid,arr,jdim)
      type(dist_grid), intent(in) :: grid
      real*8 :: arr(:,:,:,:,:)
      integer :: fid,vid,jdim
      real*8, pointer :: arrgijklm(:,:,:,:,:)
      integer :: rc
      call alloc_global(grid,arrgijklm,grid%npx,grid%npy,
     &       size(arr,3),size(arr,4),size(arr,5))
      if(grid%am_i_globalroot) then
        rc = nf_get_var_double(fid,vid,arrgijklm)
      endif
      call unpack_data(grid,arrgijklm,arr)
      deallocate(arrgijklm)
      return
      end subroutine par_read_ijxxx
      subroutine par_read_xijx(grid,fid,vid,arr,jdim)
      type(dist_grid), intent(in) :: grid
      real*8 :: arr(:,:,:,:)
      integer :: fid,vid,jdim
      real*8, pointer :: arrgkijl(:,:,:,:)
      integer :: rc
      call alloc_global(grid,arrgkijl,size(arr,1),grid%npx,grid%npy,
     &     size(arr,4))
      if(grid%am_i_globalroot) then
        rc = nf_get_var_double(fid,vid,arrgkijl)
      endif
      call unpack_data(grid,arrgkijl,arr,jdim=3)
      deallocate(arrgkijl)
      return
      end subroutine par_read_xijx
#endif /* not SERIAL_MODE */


      subroutine alloc_global1(grid,a,i1)
      type(dist_grid), intent(in) :: grid
      real*8, pointer :: a(:)
      ...
      end subroutine alloc_global1

      subroutine alloc_global2(grid,a,i1,i2)
      type(dist_grid), intent(in) :: grid
      real*8, pointer :: a(:,:)
      ...
      end subroutine alloc_global2

      subroutine alloc_global3(grid,a,i1,i2,i3)
      type(dist_grid), intent(in) :: grid
      real*8, pointer :: a(:,:,:)
      ...
      end subroutine alloc_global3

      subroutine alloc_global4(grid,a,i1,i2,i3,i4)
      type(dist_grid), intent(in) :: grid
      real*8, pointer :: a(:,:,:,:)
      integer, intent(in) :: i1,i2,i3,i4
      if (grid%am_i_globalroot) then
        allocate( a(i1,i2,i3,i4) )
      else
        allocate( a(1,1,1,1) )
      endif
      end subroutine alloc_global4

      subroutine alloc_global5(grid,a,i1,i2,i3,i4,i5)
      type(dist_grid), intent(in) :: grid
      real*8, pointer :: a(:,:,:,:,:)
      integer, intent(in) :: i1,i2,i3,i4,i5
      if (grid%am_i_globalroot) then
        allocate( a(i1,i2,i3,i4,i5) )
      else
        allocate( a(1,1,1,1,1) )
      endif
      end subroutine alloc_global5
