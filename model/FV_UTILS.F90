...
module FV_UTILS
!@sum The following module provides helper functions and subroutines to aid in the conversion of
!@+ the ESMF-wrapped version of the FV dynamical core from GEOS-5 to GISS modelE and viceversa.
!@auth Tom Clune GSFC/SSSO/610.3 and modelE development team
...
!@var FV_CORE  This data structure is o convenient entity for storing persistent data between
...
!@var rc private data of convenience (return code from ESMF)
  Integer :: rc
...
!@sum  The following routine interfaces the VERIFY_ macro with the
...
!@sum Reverse the order of vertical levels.
    Real*8, Intent(In) :: A(:,:,:)
...
!@sum Single precision variant of Reverse()
    Real*4, Intent(In) :: A(:,:,:)
...
!@sum Return GISS 3D edge pressures
...
!@sum Compute Delta-pressure for GISS model
    USE RESOLUTION, only: IM, LM, LS1
...
!@sum Convert Potential Temperature into (dry) Temperature
    USE RESOLUTION, only: IM, LM, LS1
    Use MODEL_COM, only: T
    USE DOMAIN_DECOMP_ATM, only: grid, GET
...
!@sum Compute 3D pressure (raised to the kappa)
    USE RESOLUTION, only: IM, LM, LS1
...
!@sum Convert pressure from GISS representation to FV representation.
...
!@sum Convert potential temperature between the two representations.
...
!@sum Convert potential temperature as exported by FV to the corresponding 
...
!@sum Create and initialize the ESMF clock
...
!@sum This routine uses system calls to rename  FVCORE restart files
...
!@sum Retrieve and allocate a pointer from FV's export state
...
!@sum Write instance of FV's internal state to a file
...
!@sum Create, open and load the ESMF configuration object into memory
    use ESMF
    use FILEMANAGER
...
!@sum Perform a series of calls to various routines to prepare modelE to interface
!@+ with GEOS5 FV core
    use DOMAIN_DECOMP_ATM, only : AM_I_ROOT
#ifdef CUBED_SPHERE
...
!@sum Initialize specific humidity anf geopotential in FV state
...
!@sum Generic routine to compute a tendency d/dt A
...
!@sum Zero-out the FV tendencies
    Type (FV_CORE) :: fv
...
!@sum Save tendencies into specified file unit
    use domain_decomp_atm, only: AM_I_ROOT
    use FILEMANAGER
...
!@sum Zero-out the accumulated fluxes
