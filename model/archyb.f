...
c --- check if ogcm date matches agcm date
     ...
     .     amon,0,'_out',xlabel(1:lrunid),'.nc'
        temav(:,:,:)=temp(:,:,:)
        salav(:,:,:)=saln(:,:,:)
        th3av(:,:,:)=th3d(:,:,:)
        dpav (:,:,:)=  dp(:,:,:)/onem
...
c --- check if ogcm date matches agcm date
     ...
     .     (itime+1.)/nday,time
        ...
        write(flnm_nc,'(a3,i4.4,3a)')
     .    amon,Jyear,'_out',xlabel(1:lrunid),'.nc'
      endif
c
...
c
      write (lp,'(3a,i5)') 'shown below: ',intvl
     .    ,'- day SSH average step=',nstep
      call zebra(sfhtav,idm,ii1,jj)
      write (lp,'(3a,i5)') 'shown below: ',intvl
     .    ,'- day SST average, step=',nstep
      call zebra(temav,idm,ii1,jj)
      write (lp,'(3a,i5)') 'shown below: ',intvl
     .    ,'- day ice average, step=',nstep
      call zebra(oiceav,idm,ii1,jj)
c
      if (binary_output) then
      no=no+1
...
!<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>-<>
     ...
     .    'srfht','monthly sea surface height','m')
        call out2cdf(ncid1,idm,jdm,dpmxav,time,
     .    'zmixl','monthly mixed layer depth','m')
c       call out2cdf(ncid1,idm,jdm,ticeav,time,
c    .    'tice','monthly ice surface temp','deg C')
        call out2cdf(ncid1,idm,jdm,oiceav,time,
     .    'covice','monthly ice coverage','m')
        call out2cdf(ncid1,idm,jdm,eminpav,time,
     .    'eminp','monthly precip minus evapo, pos.down','m/sec')
        call out2cdf(ncid1,idm,jdm,surflav,time,
     .    'srflx','monthly surface heatflux, pos.down','W/m2')
        call out2cdf(ncid1,idm,jdm,tauxav,time,
     .    'taux','monthly taux','N/m2')
        call out2cdf(ncid1,idm,jdm,tauyav,time,
     .    'tauy','monthly tauy','N/m2')
c       call out2cdf(ncid1,idm,jdm,radflav,time,
c    . 'radfx','monthly surface net radiative flux, pos.down','W/m^2')
c       call out2cdf(ncid1,idm,jdm,snsibav,time,
c    . 'snsib','monthly surface sensible heat flux, pos.down','W/m^2')
c       call out2cdf(ncid1,idm,jdm,latntav,time,
c    . 'latnt','monthly surface latent heat flux, pos.down','W/m^2')
        call out1cdf(ncid1,kdm,theta,0.,
     .    'theta','target pot.density, sigma1','kg/m^3')

        call out3cdf(ncid1,idm,jdm,kdm,temav,time,
     .    'temp','monthly potential temperature','deg C')
        call out3cdf(ncid1,idm,jdm,kdm,salav,time,
     .    'saln','monthly salinity','psu')
        call out3cdf(ncid1,idm,jdm,kdm,th3av,time,
     .    'th3d','monthly pot.density, sigma1','kg/m^3')
        call out3cdf(ncid1,idm,jdm,kdm,dpav,time,
     .    'thik','monthly averaged layer thickness','m')
        call out3cdf(ncid1,idm,jdm,kdm,uav,time,
     .    'uvel','monthly southward velocity','m/sec')
        call out3cdf(ncid1,idm,jdm,kdm,vav,time,
     .    'vvel','monthly eastward velocity','m/sec')
        if (ntrcr.ge.1)
     ...
     .    'uflx','monthly integral of southward mass flux','N')
        call out3cdf(ncid1,idm,jdm,kdm,vflxav,time,
     .    'vflx','monthly integral of eastward mass flux','N')
