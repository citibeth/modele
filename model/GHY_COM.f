...
C**** If the SOILIC file is present, they are allocated by read_landsurf_ic,
     ...
     &       earth_ice(0:ngm,ls_nfrac,i_0:i_1,j_0:j_1))
      ...
      do ifrac=1,ls_nfrac
        ...
        do i=i_0,i_1
          ...
          earth_sat(0,ifrac,i,j) = wetness(1,i,j)
          earth_ice(:,ifrac,i,j) = 0.
          where(temp(:,i,j).lt.0.) earth_ice(1:ngm,ifrac,i,j) = 1.
          snowbv(ifrac,i,j) = max(0d0,snowdp(i,j))/1000d0 ! kg/m2 -> m h2o
