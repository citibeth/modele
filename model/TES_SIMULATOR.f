...
C
C      Set the mnemonic 'TES_SINGLE', a netcdf file with categorical 
C      averaging kernel info for Single, should be 'Single.AvgKrnls.nc'
C       
...
C      threshold. 70 seems reasonable. Alternatively, set this to -9999
C      to randomly decide, based on the categorical quality, whether it 
C      is included.
C
...
C      profile and non-HDO avg krnls from TES_SINGLE in the
C      categorical operator. This overwrites the q 
C      priors and non-HDO avg krnls from whatever was used in 
C      TES_AK. Just for sensitivity testing - to identify 
...
C the height of maximum sensitivity. The convolution calculation is
C in 'ApplyTESAvgKrnls', based on code from Camille Risi.
C
C
C********************************************************************** 
C**********************************************************************
...
C    - also accumulate different ancillary diagnostics
C
...
C**** INTERNAL VARIABLES
      ...
      INTEGER randISCCPCol,goodProfile
...
C****             Randomly select an ISCCP subgrid column, get cod/ctp
C****             Might not even need this due to randomization within
C****             ISCCP?
...
C********************************************************************** 
      ...
      INTEGER :: nI,nJ,I,J,code,randISCCPCol
      REAL*8 xx
      ...
      DO j=J_0,J_1
          DO i=I_0,I_1
              randISCCPCol = CEILING(ncol*randu(xx))
              ISCCPRandIndex(i,j) = randISCCPCol
          ENDDO
      ENDDO
