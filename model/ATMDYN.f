...
!@+       1 global patch with mode=1 is assumed.
     ...
     &     ipatch_aflux_topo_adj,jpatch_aflux_topo_adj
      ...
      use model_com, only : imh,lm,im,jm
      use param
      implicit none
...
c
      do adjmode=0,1
        ...
        do nn=1,npatch_aflux_topo_adj
          ...
          j1p=max(jpatch_aflux_topo_adj(1,nn),j_0s)
          j2p=min(jpatch_aflux_topo_adj(2,nn),j_1s)
          do j=j1p,j2p
          ...
          j1p=max(jpatch_aflux_topo_adj(1,nn),max(3,j_0s))
          j2p=min(jpatch_aflux_topo_adj(2,nn),j_1s)
          do j=j1p,j2p
          do i=ipatch_aflux_topo_adj(1,nn),ipatch_aflux_topo_adj(2,nn)
...
!            real(r8) :: Eke_by2 = 5.5E-6_r8
     ...
     *            exp( -(C(IC,Ikh)/Cw(Ikh))**2 * aLn2 ) * rhoe(IZ0(J))
     ...
     &           , Eps, Kh(Ikh), hb, rhoe, U_Comp, bvfe, MC, IZ0(J), GW)
      ...
      real(r8) :: x, Bt_Tmax      
      real(r8) :: Bt_Nmax, Bt_Nmin    
      real(r8) :: Bt_Smax, Bt_Smin   
      real(r8) :: func_N(JDPERY+196), func_S(JDPERY) 
      real(r8) :: Phi_N(JDPERY+196), Phi_S(JDPERY)   
      real(r8) :: Bt_subTmax_N, Bt_subTmax_S  
      real(r8) :: Phi_subTNmax, Phi_subTSmax 
      real(r8) :: Bt_SubT_N(JDPERY), Bt_SubT_S(JDPERY+106)  
      real(r8) :: trans_time    
      ...
      Bt_Nmax = 3.0_r8 * 0.001_r8       
      Bt_Nmin = 1.0_r8 * 0.001_r8   
 
      Bt_Smax = 9.0_r8 * 0.001_r8   
      Bt_Smin = 1.2_r8 * 0.001_r8 

      Bt_subTmax_N = 5.5_r8 * 0.001_r8
      Bt_subTmax_S = 0.6_r8 * 0.001_r8
      Phi_subTNmax = 15.0_r8
      Phi_subTSmax = - 15.0_r8
 
      Bt_subT_N(:) = 0.0_r8   
      Bt_subT_S(:) = 0.0_r8   
      do IT = 106, 288   
      x = cos ( twopi * real(IT - 106, r8) / real(182, r8) )
      Bt_subT_N(IT) = Bt_subTmax_N / 2.0_r8 * ( 1.0_r8 - x )
      end do  
      do IT = 289, JDPERY + 106
      x = cos ( twopi * real(IT - 289, r8) / real(182, r8) )
      Bt_subT_S(IT) = Bt_subTmax_S / 2.0_r8 * ( 1.0_r8 - x )   
      end do
      Bt_subT_S(1:106) = Bt_subT_S(JDPERY+1:JDPERY+106) 

      trans_time = 60.0_r8   
      do IT = 197, 305    
      x = ( tanh ( real(IT-197,r8) / trans_time) ) 
     *  / ( tanh ( real(305-197,r8) / trans_time) ) 
      func_N(IT) = Bt_Nmin + ( Bt_Nmax - Bt_Nmin ) * x    
      Phi_N(IT) = 75.0_r8 - 15.0_r8 * x     
      end do 
      do IT = 306, JDPERY + 196      
      x = ( tanh ( real(JDPERY+197-IT,r8) / trans_time) ) 
     *  / ( tanh ( real(JDPERY+197-305,r8) / trans_time) )   
      func_N(IT) = Bt_Nmin + ( Bt_Nmax - Bt_Nmin ) * x  
      Phi_N(IT) = 75.0_r8 - 15.0_r8 * x
      end do    
      func_N(1:196) = func_N(JDPERY+1:JDPERY+196) 
      Phi_N(1:196) = Phi_N(JDPERY+1:JDPERY+196)    

      do IT = 1, JDPERY
      x = cos ( twopi * real(IT - 197, r8) / real(JDPERY, r8) )     
      func_S(IT) = ( Bt_Smax + Bt_Smin ) / 2.0_r8 + 
     *             ( Bt_Smax - Bt_Smin ) / 2.0_r8 * x  
      Phi_S(IT) = - (75.0_r8 + 60.0_r8) / 2.0_r8  
     *          + (75.0_r8 - 60.0_r8) / 2.0_r8 * x   
      end do 

      do IT = 1, JDPERY
         do J = 1, JM
            Bt(:,J,IT) = func_S(IT) *
     *      exp(-(( LAT_DG(J,2) - Phi_S(IT) )/15.0_r8)**2 * aLn2 )  
     *           + func_N(IT) *  
     *      exp(-(( LAT_DG(J,2) - Phi_N(IT) )/15.0_r8)**2 * aLn2 ) 
!     *           + 2.9_r8 * 0.001_r8 *
!     *      exp(-(LAT_DG(J,2)/10.0_r8)**2 * aLn2 ) * 
         if ( LAT_DG(J,2) < Phi_subTSmax + 1E-8_r8 ) then
            Bt(:,J,IT) = Bt(:,J,IT) + Bt_subT_S(IT) * 
     *    exp(-( ( LAT_DG(J,2) - Phi_subTSmax ) / 20.0_r8 ) **2 * aLn2 )
         else 
            Bt(:,J,IT) = Bt(:,J,IT) + Bt_subT_S(IT) *   
     *    exp(-( ( LAT_DG(J,2) - Phi_subTSmax ) / 5.0_r8 ) **2 * aLn2 )
         end if 
         if ( LAT_DG(J,2) < Phi_subTNmax + 1E-8_r8 ) then 
            Bt(:,J,IT) = Bt(:,J,IT) + Bt_subT_N(IT) *
     *    exp(-( ( LAT_DG(J,2) - Phi_subTNmax ) / 5.0_r8 ) **2 * aLn2 )
         else
            Bt(:,J,IT) = Bt(:,J,IT) + Bt_subT_N(IT) *
     *    exp(-( ( LAT_DG(J,2) - Phi_subTNmax ) / 20.0_r8 ) **2 * aLn2 )
         end if 
         end do
      end do

      Bt_Tmax = 11.8_r8 * 0.001_r8  
      do IT = 1, JDPERY
         do J = 1, JM
         do I = 1, IM
            if ( LON_DG(I,2) < -60.0_r8 - 1E-8_r8 ) then
               x = 360.0_r8 + LON_DG(I,2) 
            else
               x = LON_DG(I,2)  
            end if  
            Bt(I,J,IT) = Bt(I,J,IT) + Bt_Tmax *
     *          exp(-(LAT_DG(J,2)/10.0_r8)**2 * aLn2 ) *
     *          exp(-( ( x - 120.0_r8 ) / 60.0_r8 ) **2 * aLn2 ) 
         end do
         end do
      end do 
...
!         do I = 1, IM
!            Bt(I,J,IT) = Bt(I,J,IT) + Bt_Tmax *
!     *          exp(-( (LAT_DG(J,2) - Phi)/10.0_r8 )**2 * aLn2 ) *
!     *          0.25_r8 * ( 3.0_r8 - x )
!         end do
!         end do
!      end do 
...
!      Bt = Bt + 1.0_r8 * 0.001_r8
      ...
      I = 1
      do while ( PLbot(I) - 100.0_r8 >= -1e-6 ) 
         I = I + 1
         if ( I == LM + 2 ) exit
      end do
         IZ0(:) = I - 2            !Source is located below 100 hPa. 
      L_min = minval(IZ0)
...
!      real(r8), parameter :: byFc_sq = 0.5_r8
      real(r8), parameter :: byFc_sq = 0.25_r8  
