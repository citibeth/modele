...
!@+     RESOLUTION DEPENDENT: This version is for 288x180L32 - S
...
C****
     ...
     *    159,159,160,172,126,142,131,132,103, 92, 85/),(/NMST,2/) )
      integer, parameter ::
#ifdef OCN_LAYERING
     &  LMST32(NMST) = ! store reference 32-layer indices
#else
     &  LMST(NMST) =
#endif
     &     (/ 3, 2, 2, 6, 7, 2, 2, 2, 4, 2, 2 /)
#ifdef OCN_LAYERING
      ! Workaround (for specification of strait depths as layer
      ! indices LMST rather than physical depths) when layering
      ! differs from that corresponding to LMST above.
      real*8, dimension(7), parameter :: zel32 = ! first maxval(lmst32)
     &     (/12, 30, 56, 92, 140, 202, 280 /)    ! L32 edge depths
      real*8, dimension(nmst) :: zstl32 = zel32(lmst32)
      integer, dimension(nmst) :: lmst ! set at runtime from zstl32
#endif
