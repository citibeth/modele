...
!@auth Gavin Schmidt
    ...
    (defined TRACERS_QUARZHEM)
     ...
     &       ,tr_mm
      ...
      USE TRACER_COM , only : n_SO4,n_SO2,n_seasalt1,n_seasalt2,n_BCIA,
     * n_BCB,n_OCIA,n_silt1,n_silt2,n_silt3,SNFST0,TNFST0
...
! heights, temps, rel/spec hum, velocities/cloud opt depth on layers
            case ("Z", "T", "R", "Q", "U", "V", "W", "C")
              kunit=kunit+1
...
c           case ("Z", "T", "R", "Q")! heights, temps, rel/spec hum,PMB levels
c             do kk=1,kgz_max
c               kunit=kunit+1
c               call openunit(namedd(k)(1:1)//trim(PMNAME(kk))//
c    *               aDATE(1:7),iu_SUBDD(kunit),.true.,.false.)
c               call io_POS(iu_SUBDD(kunit),Itime-1,im*jm,Nsubdd)
c             end do
#ifdef TRACERS_SPECIAL_Shindell
            case ("O")  ! Ox tracer
              kunit=kunit+1
     ...
     *           .true.,.false.)
            kk=Nsubdd
            if(namedd(k)(1:4).eq."TMIN".or.namedd(k)(1:4).eq."TMAX")
     *      kk=Nday
            call io_POS(iu_SUBDD(kunit),Itime-1,im*jm,kk)
          endif
        end do
        kddunit=kunit
      end if
...
!@+                    SO4,SO2
!@+                    SW_TOA,LW_TOA,SW_SURF,LW_SURF
#ifdef TRACERS_HETCHEM
!@+                    SO4_d1,SO4_d2,SO4_d3,SO4_d3,
...
!@auth Gavin Schmidt/Reto Ruedy
     ...
     *     ,lhe,rhow,undef,stbo
      USE MODEL_COM, only : lm,p,ptop,zatmo,dtsrc,u,v,t,q,focean,fearth
     *     ,flice,nday
...
! estimating this from the downward x albedo, since that's already saved
     ...
     *             PEARTH*(GTEMP(1,4,I,J)+TF)**4)
        ...
        case ("LWT")            ! LW upward flux at TOA (P1) (W/m^2)
          do j=1,jm        ! sum up all cooling rates + net surface emission
            do i=1,imaxj(j)
              POCEAN=(1.-RSI(I,J))*(FOCEAN(I,J)+FLAKE(I,J))
              POICE=RSI(I,J)*(FOCEAN(I,J)+FLAKE(I,J))
              PEARTH=FEARTH(I,J)
     ...
     *               +trm(i,j,1,n_SO4_d3)
         ...
         case ("SW_TOA")
          data(:,:)=SNFST0(1,n_SO4,:,:)+SNFST0(1,n_seasalt1,:,:)+
     *SNFST0(1,n_seasalt2,:,:)+SNFST0(1,n_OCIA,:,:)+SNFST0(1,n_BCB,:,:)
     *+SNFST0(1,n_BCIA,:,:)+SNFST0(1,n_clay,:,:)+SNFST0(1,n_silt1,:,:)
     *+ SNFST0(1,n_silt2,:,:)+SNFST0(1,n_silt3,:,:)
         case ("LW_TOA")
          data(:,:)=TNFST0(1,n_SO4,:,:)+TNFST0(1,n_seasalt1,:,:)+
     *TNFST0(1,n_seasalt2,:,:)+TNFST0(1,n_OCIA,:,:)+TNFST0(1,n_BCB,:,:)
     *+ TNFST0(1,n_BCIA,:,:)+TNFST0(1,n_clay,:,:)+TNFST0(1,n_silt1,:,:)
     * + TNFST0(1,n_silt2,:,:)+TNFST0(1,n_silt3,:,:)
         case ("SW_SURF")
          data(:,:)=SNFST0(2,n_SO4,:,:)+SNFST0(2,n_seasalt1,:,:)+
     *SNFST0(1,n_seasalt2,:,:)+SNFST0(2,n_OCIA,:,:)+SNFST0(2,n_BCB,:,:)
     *+ SNFST0(2,n_BCIA,:,:)+SNFST0(2,n_clay,:,:)+SNFST0(2,n_silt1,:,:)
     * + SNFST0(2,n_silt2,:,:)+SNFST0(2,n_silt3,:,:)
         case ("LW_SURF")
          data(:,:)=TNFST0(2,n_SO4,:,:)+TNFST0(2,n_seasalt1,:,:)+
     *TNFST0(2,n_seasalt2,:,:)+TNFST0(2,n_OCIA,:,:)+TNFST0(2,n_BCB,:,:)
     *+ TNFST0(2,n_BCIA,:,:)+TNFST0(2,n_clay,:,:)+TNFST0(2,n_silt1,:,:)
     * + TNFST0(2,n_silt2,:,:)+TNFST0(2,n_silt3,:,:)
#endif
        case default
          goto 10
        end select
          ...
          do kp=1,kgz_max
            if (namedd(k)(2:5) .eq. PMNAME(kp)) then
              ...
              case ("Q")        ! specific humidity
                do j=1,jm
                do i=1,imaxj(j)
                  data(i,j)=rh_inst(kp,i,j)*qsat(t_inst(kp,i,j)+TF,
     *                 lhe,PMB(kp))
                end do
                end do
...
c           do kp=1,kgz_max
c             kunit=kunit+1
c             select case (namedd(k)(1:1))
c             case ("Z")        ! geopotential heights
c               data=z_inst(kp,:,:)
c             case ("R")        ! relative humidity (wrt water)
c               data=rh_inst(kp,:,:)
c             case ("Q")        ! specific humidity
c               do j=1,jm
c               do i=1,imaxj(j)
c                 data(i,j)=rh_inst(kp,i,j)*qsat(t_inst(kp,i,j)+TF,
c    *                 lhe,PMB(kp))
c               end do
c               end do
c             case ("T")        ! temperature (C)
c               data=t_inst(kp,:,:)
c             end select
            kunit=kunit+1
            do kp=1,LmaxSUBDD
              ...
              case ("R")        ! relative humidity
                do j=1,jm
                do i=1,imaxj(j)
                  data(i,j)=q(i,j,kp)/qsat(t(i,j,kp)*pk(kp,i,j),lhe,
     *                 pmid(kp,i,j))
                end do
                end do
              case ("Q")        ! specific humidity
              ...
              case ("T")        ! temperature
                do j=1,jm
                do i=1,imaxj(j)
                   data(i,j)=T(i,j,kp)*pk(kp,i,j)-TF
                end do
                end do
              end select
C**** fix polar values
...
!@auth Reha Cakmur/Jan Perlwitz
      ...
      USE radncb,ONLY : srnflb_save,trnflb_save,ttausv_save,
     &     ttausv_cs_save
      USE dagcom,ONLY : adiurn_dust,ndiupt,ijdd,adiurn
#ifdef TRACERS_DUST
