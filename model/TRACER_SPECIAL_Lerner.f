...
C     Loss rates
     ...
     *  (((O3trop_Loss(i,j,l,m),i=1,im),j=1,jm),l=1,lmtc)
      ...
      call closeunit(iu)
      write(6,*) TITLCH
      WRITE(6,'(1X,A,I5)') 'O3 trop loss rates read'
C     Production rates
     ...
     *  (((O3trop_Prod(i,j,l,m),i=1,im),j=1,jm),l=1,lmtc)
      ...
      call closeunit(iu)
      write(6,*) TITLCH
      WRITE(6,'(1X,A,I5)') 'O3 trop production rates read'
C     Deposition velocities
      ...
      call closeunit(iu)
      write(6,*) TITLCH
      WRITE(6,'(1X,A,I5)') 'O3 deposition velocities read'
...
cc        TAIJS(I,J,26) = TAIJS(I,J,26) + (rprod-rloss)
...
C**** dz = -dP/rhoG; rho=PRT; Deposition velocity
     ...
     *                -trmom(mz,I,J,L,n)+trmom(mzz,I,J,L,n))
          ...
          if(trm(i,j,l,n) + dmass .lt.0.) then
            write(6,'(a,3i3,2f14.0,3E12.3,f9.0)')
...
cXXXXX DSOL NOT USED XXXXX
      ...
      USE DOMAIN_DECOMP, only: GRID, GET
      USE DYNAMICS, only: pk,am          ! Air mass of each box (kg/m^2)
      USE GEOM, only: imaxj,dxyp
      USE TRACER_COM
      USE PRATHER_CHEM_COM, only: nstrtc
      USE LINOZ_CHEM_COM, only: tmmvv,tlT0M,TLTZM,TLTZZM,dsol
      USE FLUXES, only: tr3Dsource
      implicit none
...
CXXX        dersol = tlT0M(j,lr,8)/tmmvv(n)*am(l,i,j)*dxyp(j)
...
C**** 1995 CH4 Concentrations in ppb; 1995 CO2 Concentrations in ppm
      USE MODEL_COM, ONLY: im,jm,lm,ls1,sige,psf,ptop,jmon0,amonth
      USE DOMAIN_DECOMP, only: GRID, GET
      USE DYNAMICS, only: pedn
      USE FILEMANAGER, only: openunit,closeunit
      implicit none
