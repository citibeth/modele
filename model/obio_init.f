...
#ifdef Relax2SurfN
        call bio_surfNg('nitrates_inicond',fldo2,fldoz)
        surfN=fldo2(:,:,1)
#endif
#else
...
!--------------------------------------------------------------
      ...
      subroutine bio_surfNg(filename,fldo,dateline)

!read in a field at 1x1 resolution
!convert to atmospheric grid
!convert to ocean grid 
!this routine only for (i,j) arrays
      ...
      real data(igrd,jgrd)
      real data_min,data_max
      real data_mask(igrd,jgrd)
      real fldo(ogrid%I_STRT_HALO:ogrid%I_STOP_HALO,
     .          ogrid%J_STRT_HALO:ogrid%J_STOP_HALO)
      real fldo_glob(idm,jdm)
...
!--------------------------------------------------------------
      ...
      do j=1,jgrd
        !mask
        data_mask(i,j)=0.d0
        if (data(i,j)>=0.d0) data_mask(i,j)=1.d0
      enddo    ! i-loop
      enddo    ! j-loop
     ...
     .             idm,jdm,0.d0,oDLATM,-9999.d0)
      ...
      call HNTR8P (data_mask,data(:,:),fldo_glob(:,:))

      do i=1,idm
      do j=1,jdm
        if (fldo_glob(i,j)<=0.d0) fldo_glob(i,j)=data_min
      enddo    ! i-loop
      enddo    ! j-loop
