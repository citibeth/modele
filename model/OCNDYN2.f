...
C****
      ...
      USE MODEL_COM, only : modd5s,msurf,itime, DTSRC
      USE OCEAN, only : im,jm,lmo,ndyno,nocean,mo,g0m,
     *    s0m,dts,dtofs,dto,dtolf,mdyno,msgso,
...
C**** Apply bottom and coastal drags
      If (OBOTTOM_DRAG /= 0 .and. OBOTTOM_DRAG /= 2)  Call OBDRAG2
      If (OBOTTOM_DRAG == 2)  Call OWALL
      If (OCOASTAL_DRAG /= 0 .and.
     *    (OCOASTAL_DRAG /= 2 .or. OBOTTOM_DRAG /= 2))  Call OCOAST
...
c
      if(use_qus==1) then
        CALL OADVT3 (G0M,
     &     GXMO,GYMO,GZMO, GXXMO,GYYMO,GZZMO, GXYMO,GYZMO,GZXMO,
     &     DTOLF,.FALSE.,OIJL(1,J_0H,1,IJL_GFLX))
        CALL OADVT3 (S0M,
     &     SXMO,SYMO,SZMO, SXXMO,SYYMO,SZZMO, SXYMO,SYZMO,SZXMO,
     &     DTOLF,.TRUE.,OIJL(1,J_0H,1,IJL_SFLX))
      else
     ...
     *        ,OIJL(1,J_0H,1,IJL_SFLX))
     ...
     &       DTOLF,t_qlimit(n),TOIJL(1,J_0H,1,TOIJL_TFLX,N))
        ENDDO
      else
        DO N=1,NTM
...
C****
C**** FFT-based filter for all-ocean segments
C****
...
C**** the matrices needed for the latitudes on this processor.
     ...
     *          REDUCO_glob(INDM))
      ...
      If (AM_I_ROOT())  Write (6,*) 'Read unit',IU_AVR,': ',TITLE
...
#ifdef  apply_WindStressAnomaly
!at the end of the routine OSTRESS we need to subtract fafmip anomalies so that stress is what other
!parts of the model see: look below "restore ..."
      DO J=J_0S,J_1S
      DO I=1,IMO
      oDMUA(I,J,1) = oDMUA(I,J,1) + FAFMIP_tauu(I,J)*dtsrc !do we need to multiply fafmip with open ocean water?
                           !do we also need to multiply by timestep? Are the units correct?
      oDMVA(I,J,1) = oDMVA(I,J,1) + FAFMIP_tauv(I,J)*dtsrc
      ENDDO
      ENDDO
#endif
