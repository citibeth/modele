...
c
     ...
     &     )
      arr(next()) = info_type_(
     &  sname = 'aod_cs_3d',
     &  lname = 'AOD CS on model levels',
     &  units = '-',
     &  sched = sched_rad
     &     )
      arr(next()) = info_type_(
     &  sname = 'aaod_cs_3d',
     &  lname = 'AAOD CS on model levels',
     &  units = '-',
     &  sched = sched_rad
     &     )
      ...
      do n=1,nraero
        ...
        if (trim(trname(ntrix(n))).eq.'Clay') then
          nntrace=nntrace+1
          write(ntrace_str,'("'//trim(trname(ntrix(n)))//
     &                     '",i1)')nntrace
        else
          ntrace_str=trim(trname(ntrix(n)))
        endif
        arr(next()) = info_type_(
     &    sname = 'aod_'//trim(ntrace_str)//'_3d',
     &    lname = trim(ntrace_str)//' AOD on model levels',
     &    units = '-',
     &    sched = sched_rad
     &       )
        arr(next()) = info_type_(
     &    sname = 'aaod_'//trim(ntrace_str)//'_3d',
     &    lname = trim(ntrace_str)//' AAOD on model levels',
     &    units = '-',
     &    sched = sched_rad
     &       )
        arr(next()) = info_type_(
     &    sname = 'aod_cs_'//trim(ntrace_str)//'_3d',
     &    lname = trim(ntrace_str)//' AOD CS on model levels',
     &    units = '-',
     &    sched = sched_rad
     &       )
        arr(next()) = info_type_(
     &    sname = 'aaod_cs_'//trim(ntrace_str)//'_3d',
     &    lname = trim(ntrace_str)//' AAOD CS on model levels',
     &    units = '-',
     &    sched = sched_rad
     &       )
      enddo
...
!@sum set_subdd_period get index of the current subdd accumulation period
      use model_com, only : itime,itimei,jdate,nday,dtsrc
