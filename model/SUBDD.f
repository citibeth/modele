...
!@+     "i" (e.g. varname:i or varname:NNi).  The number of model
!@+     timesteps per day (Nday) must be divisible by Nsubdd.
!@+
...
!@var write_daily_files flag indicating whether to output data
!@+   once per day (the default is once per month)
      logical :: write_daily_files
!@var max_days_per_file==1 if write_daily_files, otherwise ==31
      integer :: max_days_per_file
...
!@var number of outputs in this group
      integer :: ndiags=0
...
!@+     output fields
      integer, parameter :: namedd_strlen=10
...
c
     ...
     &     diaglist,listlen,ndiags,dsize3_input)
      ...
      use model_com, only : lm ! temporary?
...
c
     ...
     &         'the denominator for subdd output '//trim(name_tmp(k))
      ...
      ! enough space for 1 day or a 31-day month
      nperiod = max_days_per_file*nday/nsubdd
...
!@auth M. Kelley
      ...
      use domain_decomp_atm, only : get,grid
      use param
      use subdd_mod, only : write_daily_files,max_days_per_file,
     &     vinterp_using_timeavgs,
...
C**** Note: for longer string increase MAX_CHAR_LENGTH in PARAM
     ...
     & subdd1=" ", subdd2=" ", subdd3=" ", subdd4=" ", subdd5=" ",
     & subdd6=" "
!@var subddt = subdd + subdd1,2,3 = all variables for sub-daily diags
      CHARACTER*454 :: subddt = " " ! remember to add room for spaces!
      ! e.g. here, 454=7*64+6
...
!@var kdd total number of sub-daily diags
      ...
      integer, dimension(8), parameter :: allowed_hrfreqs=
     &     (/ 1, 2, 3, 4, 6, 8, 12, 24 /)
      integer, dimension(9) :: allowed_freqs
     ...
     &     vinterp_using_timeavgs_int)
      ...
      allowed_freqs(1) = 1
      allowed_freqs(2:) = allowed_hrfreqs(:)*(nday/24)
...
c
     ...
     *     ("Increase kddmax: No. of sub-daily diags too big",255)
      ...
      do k=1,kdd
        if(.not.any(allowed_freqs.eq.ddfreq(k))) then
          call stop_model ("parse_subdd: invalid frequency",255)
        endif
      enddo
...
c
      ngroups = 0
      do ifreq=1,size(allowed_freqs)
      do avg_and_inst=1,2
        instcat = avg_and_inst == 2
        kdd_thisfreq = 0
        do k=1,kdd
          if(ddfreq(k).ne.allowed_freqs(ifreq)) cycle
          if(instcat) then
...
c
        write(c2,'(i2)') allowed_freqs(ifreq)
        c3 = adjustl(c2)
        if(instcat) c3=trim(c3)//'i'
c
        do idcat=1,ncats_max
          catshape = catshapes(idcat)
          if(len_trim(catshape).eq.0) exit
          grpname = trim(categories(idcat))//trim(c3)
          call create_group(
     &         subdd_groups(ngroups+1),allowed_freqs(ifreq),
...
C**** interpolate to pressure levels and accumulate the subdd diagnostics
     ...
     &      ,subdd_npres,subdd_pk, subdd_pres
      ...
      USE DYNAMICS, only : ualij,valij,gz,wsave,pk,pmid
      use domain_decomp_atm, only : grid,get
      USE MODEL_COM, only: p,u,v,t,q
      USE MODEL_COM, only: zatmo,ptop,lm
      USE GEOM, only: imaxj
...
C
      case ('slp')
        do j=j_0,j_1; do i=i_0,imaxj(j)
          ...
          sddarr2d(i,j) =
     &         slp(p(i,j)+ptop,tsavg(i,j),bygrav*zatmo(i,j))
        enddo;        enddo
        call inc_subdd(subdd,k,sddarr2d)
      end select
      enddo
...
!@sum set_subdd_period get index of the current subdd accumulation period
      use model_com, only : itime,jdate,nday
      use subdd_mod, only : sched_src,subdd_type,write_daily_files
      implicit none
      integer :: istep,subdd_period
      type(subdd_type) :: subdd
      istep = mod(itime,nday)
      if(.not.write_daily_files) then
        istep = (jdate-1)*nday + istep
      endif
      subdd_period = 1+istep/subdd%nsubdd
      subdd%subdd_period = subdd_period
      if(subdd%is_inst) then
        subdd%accum_this_step = mod(1+itime,subdd%nsubdd)==0
      else
     ...
     &       subdd%nacc(subdd_period,sched_src) + 1
      ...
      subroutine get_subdd_time_info(subdd,timenow,dtime)
!@sum set_subdd_time_info get index of the current subdd accumulation period,
!@+   counted starting from the beginning of a simulation
      use model_com, only : itime,itimei,DTsrc,nday
      use subdd_mod, only : subdd_type
      implicit none
      type(subdd_type) :: subdd
      real*8 :: timenow,dtime
      integer :: n
      if(subdd%nsubdd.gt.0) then
        n = 1+(itime-itimei-1)/subdd%nsubdd
        dtime = DTsrc*subdd%nsubdd/3600d0
        if(subdd%is_inst) then
          timenow = (real(n,kind=8)   )*dtime
        else
          timenow = (real(n,kind=8)-.5)*dtime
        endif
      else
        timenow = 0.
        dtime = 0.
      ...
      return
      end subroutine get_subdd_time_info
...
!@sum reset_cached_subdd resets cached_subdd accumulations
     ...
     &     ,reduc_min,reduc_max
      ...
      use diag_com, only : sname_strlen,lname_strlen
      implicit none
      integer :: fid
      logical :: r4_on_disk
      character(len=sname_strlen) :: grpname
     ...
     &           r4_on_disk=r4_on_disk)
      ...
      use model_com, only : im,jm,ioread,iowrite,iowrite_single
      use subdd_mod, only : subdd_groups,subdd_ngroups
     ...
     &           subdd_groups(k)%v5d)
      ...
      use domain_decomp_atm, only : get,grid,hasSouthPole,hasNorthPole
      use diag_com, only : sname_strlen
      implicit none
      integer :: kdn,kup
      real*8 :: wtdn,wtup
     ...
     &             +wtup*subdd_groups(n)%v5d(i,j,kup,l,m)
      ...
      use model_com, only : jmon,jdendofm,idacc
      use diag_com, only : monacc,sname_strlen
      use subdd_mod, only : subdd_type,write_daily_files
...
c
     ...
     &           subdd%nperiod*(jdendofm(k)-jdendofm(k-1))/31
      ...
      call get_subdd_time_info(subdd,timelast,dtime)
      if(idacc(1).gt.1) then  ! normal end-of-month write
        time_axis(nperiod_thismo) = timelast
        do k=nperiod_thismo-1,1,-1
          time_axis(k) = time_axis(k+1) - dtime
        enddo
      else                    ! IC write
        time_axis(1) = timelast
