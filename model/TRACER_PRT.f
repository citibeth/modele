...
!@ver  1.0
      ...
      USE MODEL_COM, only: ITIME,NDAY,JDATE,JYEAR,JMON,JHOUR
...
#ifdef TES_SIM
C**** Apply TES operator to H2O and HDO profiles, and accumulate
C**** diagnostics. This is done in TRACEA, rather than clouds, for eg,
C**** for consistency with taijl. The new diagnostics are accumulated
C**** in aijl, because the taijl arrays (where they should be) don't
C**** have a 'denom_taijl' array, needed for conditional accumulation.
C**** Categorical averaging kernels have been read in in init_tracer of
C**** TRACERS_DRV with the call to ReadTESAvgKrnlFile.
      CALL InitializeISCCPRandIndices(I_0,I_1,J_0,J_1)
      CALL InitializeTESSUBDD(grid%I_STRT_HALO,grid%I_STOP_HALO,
     &                      grid%J_STRT_HALO,grid%J_STOP_HALO)

      IF (TESObsDiagnosticsOn.eq.1) THEN

C****     Initialize collocated TES operator only if it's a new day
          IF (MOD(ITIME,NDAY).eq.0) THEN
              CALL ReadDailyTESFile(JYEAR,JMON,JDATE,localNObs)
              print *,'localNObs: ',localNObs
C****         Proceed only if data exists for new day
              if (ALLOCATED(TESPresPt)) then
C****              if (localNObs > 0) then    
                  CALL InitializeTESOperatorCollocation(
     &                 I_0,I_1,J_0,J_1,localNObs)
              end if
          END IF
C****          if (localNObs > 0) then
              CALL ApplyTESOpRetrievalAK(J_0,J_1,I_0,I_1)
C****          end if
      END IF

      IF (TESCatDiagnosticsOn.eq.1) THEN
C****           if (localNObs > 0) then
           CALL ApplyTESOpCategoricalAK(J_0,J_1,I_0,I_1)
#ifdef CACHED_SUBDD
           ...
           case('nTES')
               call inc_subdd(subdd,k,nTESSUBD)
           case('nTESGoodC')
               call inc_subdd(subdd,k,nTESCGoodSUBD)
           case('nTESGoodR')
               call inc_subdd(subdd,k,nTESRGoodSUBD)
           case('HDORaw')
               call inc_subdd(subdd,k,HDORawSUBD)
           case('H2ORaw')
               call inc_subdd(subdd,k,H2ORawSUBD)
           case('HDOC') 
               call inc_subdd(subdd,k,nTESCGoodSUBD*HDOCSUBD)
           case('H2OC')
               call inc_subdd(subdd,k,nTESCGoodSUBD*H2OCSUBD)
           case('HDOR')
               call inc_subdd(subdd,k,nTESRGoodSUBD*HDORSUBD)
           case('H2OR')
               call inc_subdd(subdd,k,nTESRGoodSUBD*H2ORSUBD)           
           end select
           enddo
           enddo
C****           endif
