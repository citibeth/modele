...
!     . ,gmelt,egmelt,solar,gtempr,erunpsi
     ...
     &     tracflx_loc=>tracflx
! tracflx needs local ,
! scatter after call fld_a2o(atracflx(:,:,nt),tracflx(:,:,nt))
...
c
     ...
     .            'A','B','C','D','E','F','G','H','I','J'/
c     data iatest,jatest/31,41/	! Iceland
      data iatest,jatest/36,45/	! cold tongue
      data nflip/0/
c
...
#ifdef TRACERS_GASEXCH_ocean
      do nt=1,ntm
        call fld_a2o(atracflx_loc(:,:,nt),tracflx_loc(:,:,nt)) !tracer flux
      enddo
#endif
#ifdef TRACERS_OceanBiology
...
c     if (Jhour.eq.23.and.nsub.eq.nstepi) diagno=.true. ! end of day
c
      if (nstep.eq.1) then
...
c    .  ' qqoutput tx,ty,ht,ep,t,s,ice,dp,day=',time,nstep
...
c      endif
      ...
      call fld_o2a(osst_loc,asst_loc)
      call tempr_o2a(osst_loc,atempr_loc)
...
#ifdef CUBED_SPHERE
      call vec_o2i(usf_loc,vsf_loc,uosurf_4dynsi_loc,vosurf_4dynsi_loc)
#else
...
#ifdef TRACERS_GASEXCH_ocean
      do nt=1,ntm
         call fld_o2a(otrac_loc(:,:,nt),atrac_loc(:,:,nt))
...
#endif
#ifdef TRACERS_OceanBiology
      call fld_o2a(tot_chlo_loc,achl_loc)
#endif
c
c     call findmx(ipa,asst,iia,iia,jja,'asst')
c     call findmx(ipa,sss,iia,iia,jja,'osss')
...
c    . iatest,jatest,' qqoutput uo,vo,tx,ty,flx,focean day=',time,nstep
c    . ,((uosurf_loc(i,j)*100.,i=iatest-2,iatest+2)
c    . , (vosurf_loc(i,j)*100.,i=iatest-2,iatest+2),j=jatest+2,jatest-2,
c    .                                                               -1)
c    . ,((ataux_loc(i,j)*100.,i=iatest-2,iatest+2)
c    . , (atauy_loc(i,j)*100.,i=iatest-2,iatest+2),j=jatest+2,jatest-2,
c    .                                                              -1)
c    . ,((aflxa2o_loc(i,j), i=iatest-2,iatest+2)
c    .,(focean_loc(i,j)*100,i=iatest-2,iatest+2),j=jatest+2,jatest-2,-1)
c
