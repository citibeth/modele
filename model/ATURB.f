...
!@sum updates the mean velocity components, potential temperature,
...
!@var call_diag logical variable whether dout is called
      ...
      USE CONSTANT, only : grav,deltx,lhe,sha,by3,teeny,mb2kg,omega2
      USE MODEL_COM, only :
...
cc      USE SOMTQ_COM, only : tmom,qmom
      USE GEOM, only : imaxj,byaxyp,axyp,sinlat2d
     ...
     &  ,lmonin_min,lmonin_max
      USE PBLCOM, only : tsavg,qsavg,uflux,vflux,tflux,qflux
...
c     real*8, parameter :: qmin=1.d-12,pblp_max=600.!mb
...
c         if(lmonin.ge.0.d0.and.lmonin.lt.lmonin_min) lmonin=lmonin_min
     ...
     &        ,kh,kq,km,ke,wt,wq,w2,uw,vw,wt_nl,wq_nl
#ifdef TRACERS_ON
     &        ,trflx,wc_nl,nta
#endif
     &        ,lm)
...
c diffuse velocities on the primary grid in single-column model
          ...
          u0(:) = u(:)
          v0(:) = v(:)
          flux_bot=rhoe(1)*uflux1(i,j)
          call de_solver_main(u,u0,km,p4,
     &       rhoebydz,bydzerho,flux_bot,flux_top,dtime,lm,.false.)
          flux_bot=rhoe(1)*vflux1(i,j)
...
!@sum computes the 3-d finite difference dz and dze
...
!@sum writes out monitoring diagnostics at i=itest,j=jtest
!@+  if call_diag=.true.
!@auth  Ye Cheng/G. Hartke
...
!@var n number of layers
     ...
     &    ,cgh1,cgq1,km_n,kh_n,pr,cgh,cgq,zet,phih,phim,by_phim,wm,kz
      integer :: j  !@var j loop variable
...
c       cgq1=7.2d0*wstar*(-qflx)/(wm1**2*dbl)
        cgq1=0.d0
        ...
        phih1=0.;by_phim1=0.;wm1=0.;pr1=0.;cgh1=0.;cgq1=0.
#ifdef TRACERS_ON
        do nt=1,nta
          cgtr(nt)=0.
        end do
